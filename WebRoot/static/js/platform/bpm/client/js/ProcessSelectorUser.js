...
function doUserSelectCheck(selectedUsers){
	if(conditions.isBranch()){ //分支 选人判断
		...
		}else if(typeof(type) != 'undefined' && type != null && ( type == "dotransmit" || type == "dosupersede" || type == "doadduser" || type == "doadduserandsubmit" || type == "doglobaljump" || type == "doglobalreader" || type == "dotaskreader" || type == "doglobaltransmit" || type == "doretreattodraft" || type == "dosupplement")){//转发，转办,增发,跳转, 全局读者，全局转发,退回拟稿人,补发
		 	if (selectedUsers == null || selectedUsers == "" || selectedUsers == "[]") {
	        	$.messager.alert('提示','请选择流程处理人。','error');
	            return false;
	         }
		...
		$.ajax({
			   ...
			   success: function(msg){
			     if(msg.workHandUsersList){
			    	...
			    	var usd = new UserSelectDialog('workflowWorkhandDialog','550','300',getPath2() + '/platform/user/bpmSelectUserAction/getWorkHandSelectUser?userList=' + encodeURI(encodeURI(JSON.stringify(msg.workHandUsersList))),'工作移交选人窗口');
			 		var buttons = [{
			 			text:'提交',
			 			id : 'workhandSubmit',
			 			//iconCls : 'icon-submit',
			 			handler:function(){
			 				...
			 				var data = "instanceId=" + processInstanceId +"&taskId=" + taskId + "&outcome=" + outcome + "&executionId="+executionId+"&userJson="+seleUserJson;
