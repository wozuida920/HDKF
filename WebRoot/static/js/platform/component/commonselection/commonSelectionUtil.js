...
CommonSelector = function(selectType,dialogDivId,targetIdEleId,targetNameEleId,userDeptIdEleId,userDeptNameEleId,extParameter,singleSelect,splitChar,dialogTitle,dialogWidth,dialogHeight){
	...
	this.init = function(isInitIcon,callBackFunc,multipleOrg,afterCloseCallFun,displaySubDeptUser){
		...
		var context =getPath2() ;
        if(isInitIcon==null || typeof(isInitIcon)=='undefined' || isInitIcon==true){
        	 //创建image元素
    		var imageObj = jQuery('<span></span>').attr('class', "input-right-icon").attr('id', targetNameEleId+"-iconspan");
    		imageObj.on("click",function(){
    			if(selectType=='user'){
    				...
    				selectOrg(dialogDivId,targetIdEleId,targetNameEleId,dialogTitle,dialogWidth,dialogHeight,extParameter,singleSelect,splitChar,callBackFunc,afterCloseCallFun,multipleOrg);
    			}
    		}).bind('mouseenter', function(){
    			$(this).addClass('input-right-icon-hover');
    		}).bind('mouseleave', function(){
    			$(this).removeClass('input-right-icon-hover');
    		});
	    	 var targetElement =  jQuery("#"+targetNameEleId);
	    	 //参数，宽度是否百分比
	    	 var isWidthPercent = false;
	    	 if(targetElement.length > 0){
	    		 var targetWidth = targetElement[0].style.width;
	    		 if(targetWidth){
	    			 var patt = new RegExp(/^\d+\.{0,1}\d+%$/);
		    		 var result = patt.test(targetWidth);
		    		 if(result){
		    			 isWidthPercent = true;
		    		 }
	    		 }
	    	 ...
	    	 if(isWidthPercent){
	  		...
	  		//宽度是百分比，否则走原来代码
	  		if(isWidthPercent){
	  			//将最外面div宽度设置为原input的宽度百分比，同时设置input的宽度为100%
	  			divObj.css("width", targetElement[0].style.width);
...
function selectUser(dialogDivId,userIdEleId,userNameEleId,deptIdEleId,deptNameEleId,dialogTitle,dialogWidth,dialogHeight,extParameter,singleSelect,splitChar,callBackFunc,afterCloseCallFun,multipleOrg,displaySubDeptUser){
	    ...
	    var reqUrl=getPath2() + '/platform/commonSelection/commonSelectionController/getOrg'+queryParam;
...
function selectDept(dialogDivId,deptIdEleId,deptNameEleId,dialogTitle,dialogWidth,dialogHeight,extParameter,singleSelect,splitChar,callBackFunc,afterCloseCallFun,multipleOrg,displaySubDeptUser){
	    ...
	    var deptUrl=getPath2() + '/platform/commonSelection/commonSelectionController/getOrg'+queryParam;
	    if(afterCloseCallFun != null && typeof(afterCloseCallFun) != 'undefined'){
	    	afterCloseCallFun = afterCloseCallFun+"()";
	    }
 	 ...
 	 * @param splitChar            可选  返回值分割符 默认逗号（,） 
 	 */
 	function selectOrg(dialogDivId,orgEleId,orgNameEleId,dialogTitle,dialogWidth,dialogHeight,extParameter,singleSelect,splitChar,callBackFunc,afterCloseCallFun,multipleOrg){
 		    var historyOrgId = "";
 		    if(orgEleId!=null && typeof(orgEleId)!='undefined'){
 		    	historyOrgId = document.getElementById(orgEleId).value;
 		    }
 		    var queryParam='?singleSelect='+singleSelect +"&selectType=org&historyId="+historyOrgId+"&extParameter="+extParameter ;
 		    if(multipleOrg != null && typeof(multipleOrg) != 'undefined'){
 		    	queryParam += "&multipleOrg="+multipleOrg ;
 		    }
 		    var orgUrl=getPath2() + '/platform/commonSelection/commonSelectionController/getOrg'+queryParam;
 		    if(afterCloseCallFun != null && typeof(afterCloseCallFun) != 'undefined'){
 		    	afterCloseCallFun = afterCloseCallFun+"()";
 		    }
 		 	var orgCommonDialog = new CommonSelectorDialog(dialogDivId,dialogWidth,dialogHeight,orgUrl,dialogTitle,afterCloseCallFun);
 	 		var buttons = [
 	 		   {
 		 			text:'确定',
 		 			id : 'OrgprocessSubimt'+new Date().getTime(),
 		 			//iconCls : 'icon-ok',
 		 			handler:function(){
 		 				var frmId = jQuery('#'+dialogDivId+' iframe').attr('id');
 		 				var frm = document.getElementById(frmId).contentWindow;
 		 				...
 		 				//选择数据提交前控制
 		 				if(!submitLimitCheck(singleSelect,resultData.length,"org")){
 		 					return ;
 		 				}
 		 			   var funRetValue = null;
 		 			   if(callBackFunc!=null && callBackFunc!='undefined'){
 		 				   ...
 		 				   if(typeof(callBackFunc) === 'function'){
 			 				...
 			 				var orgName = "" ;
 			 				for(var i=0 ; i<resultData.length;i++){
 			 					var org = resultData[i];
 			  					if(orgEleId != null && typeof(orgEleId) != 'undefined'){
 			  						if(orgId==""){
 			  	  						orgId = org.orgId;
 			  	 					  }else{
 			  	 						  orgId +=splitChar+ org.orgId;  
 			  	 					  }
 			  					}
 			  					if(orgNameEleId != null && typeof(orgNameEleId) != 'undefined'){
 			  						if(orgName==""){
 			  	  						orgName = org.orgName;
 			  	 					  }else{
 			  	 						  orgName +=splitChar+ org.orgName;  
 			  	 					  }
 			  					}
 			 				}
 							if(orgEleId != null && typeof(orgEleId) != 'undefined'){
 								jQuery("#"+orgEleId).val(orgId);
 							}
 							if(orgNameEleId != null && typeof(orgNameEleId) != 'undefined'){
 								jQuery("#"+orgNameEleId).val(orgName);
 							}
 		 	   			}
 		 			    if(funRetValue==null || typeof(funRetValue)=="undefined" || funRetValue==true){
 		 			    	orgCommonDialog.close();
 		 				}
 	 			}
 	 		}];
 	 		orgCommonDialog.createButtonsInDialog(buttons);
 	 		orgCommonDialog.show();
 	 	};
/**
 * 
 * @param dialogDivId
...
function selectRole(dialogDivId,roleIdEleId,roleNameEleId,dialogTitle,dialogWidth,dialogHeight,extParameter,singleSelect,splitChar,callBackFunc,afterCloseCallFun,multipleOrg){
	 ...
	 var roleUrl=getPath2() + '/avicit/platform6/modules/system/commonpopup/RoleSelect.jsp'+queryParam;
	 if(afterCloseCallFun != null && typeof(afterCloseCallFun) != 'undefined'){
	    	afterCloseCallFun = afterCloseCallFun+"()";
	 }
...
function selectGroup(dialogDivId,groupIdEleId,groupNameEleId,dialogTitle,dialogWidth,dialogHeight,extParameter,singleSelect,splitChar,callBackFunc,afterCloseCallFun,multipleOrg){
	...
	var groupUrl=getPath2() + '/avicit/platform6/modules/system/commonpopup/GroupSelect.jsp'+queryParam;
	if(afterCloseCallFun != null && typeof(afterCloseCallFun) != 'undefined'){
	    	afterCloseCallFun = afterCloseCallFun+"()";
	}
...
function selectPosition(dialogDivId,positionIdEleId,positionNameEleId,dialogTitle,dialogWidth,dialogHeight,extParameter,singleSelect,splitChar,callBackFunc,afterCloseCallFun,multipleOrg){
	...
	var positionUrl=getPath2() + '/avicit/platform6/modules/system/commonpopup/PositionSelect.jsp'+queryParam;
	if(afterCloseCallFun != null && typeof(afterCloseCallFun) != 'undefined'){
	    	afterCloseCallFun = afterCloseCallFun+"()";
	}
...
function createCommonQueryDialog(dialogDivId,dialogIframeId,selectType){
	var queryUrl=getPath2() + '/avicit/platform6/modules/system/commonpopup/commonSelectorQuery.jsp?selectType='+selectType ;
	var dialogTitle = "查询" ;
	if(selectType=='role'){
		dialogTitle = "角色查询" ;
	}else if(selectType=='group'){
...
ComprehensiveSelector = function(targetNameEleId,selectedDataMappingConfig,dialogDivId,dialogTitle,dialogWidth,dialogHeight,extParameter,splitChar){
	...
	this.init = function(isInitIcon,callBackFun,multipleOrg,afterCloseCallFun,displaySubDeptUser){
		...
		var context =getPath2() ;
		 //创建image元素
		var imageObj = jQuery('<span></span>').attr('class', "input-right-icon");
		imageObj.on("click",function(){
...
function selectDataComprehensively(targetNameEleId,callBackFun,dialogDivId,selectedDataMappingConfig,dialogTitle,dialogWidth,dialogHeight,extParameter,splitChar,multipleOrg,afterCloseCallFun,displaySubDeptUser){
	...
	var positionUrl=getPath2() + '/avicit/platform6/modules/system/commonpopup/ComprehensiveSelector.jsp'+queryParam;
