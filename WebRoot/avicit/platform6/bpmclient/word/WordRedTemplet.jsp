...
<form id="" method="post" enctype ="multipart/form-data">
	...
	var baseurl = '<%=request.getContextPath()%>';
		...
		//打开正文
		function doOpen() {
	        TANGER_OCX_OpenDoc("<%=templetId%>");//打开正文
		...
		function TANGER_OCX_OpenDoc(templetId) {
			...
			if(templetId != ""){
				TANGER_OCX_OBJ.BeginOpenFromURL("<%=path%>/platform/bpm/clientbpmwordction/getWordTemplet.doc?templetId="+templetId);
			}else{
		...
		function ShowTitleBar() {
			...
			TANGER_OCX_DoTaoHong("<%=path%>/platform/bpm/clientbpmwordction/getword.doc?processInstanceId=<%=processInstanceId%>&openType=wordRedTemplet");
		...
		function TANGER_OCX_DoTaoHong(URL) {
		    try {
		    	TANGER_OCX_OBJ.ActiveDocument.BookMarks("taohong").Select();
		    } catch(err) {
