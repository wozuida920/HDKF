	...
	var baseurl = '<%=request.getContextPath()%>';
		...
		function TANGER_OCX_OpenDoc(processInstanceId) {
			...
			if(processInstanceId != ""){	
				TANGER_OCX_OBJ.BeginOpenFromURL("<%=path%>/platform/bpm/clientbpmwordction/getword.doc?processInstanceId=<%=processInstanceId%>&openType=wordEdit");
			}else{
		...
		function getWordSealList() {
			...
			var usd = new UserSelectDialog("worddialog","400","300",url ,"印章列表");
			usd.show();
		}
		...
		function doWordSeal(wordSealId) {
			...
			if (wordSealId != null && wordSealId != "close") {
				AddSecSignFromURL("<%=path%>/platform/bpm/clientbpmwordction/getWordSeal.png?wordSealId="+wordSealId);
			}
		}
	</script>
