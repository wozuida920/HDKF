				...
				<c:forEach items="${rows}" var="todo" varStatus="vs">
					<tr>
						...
						<!-- 
						 ...
						 -->
						<td _id="rmie6"  width="20%" class="alignl" title="${todo.processDefName }"  ><c:if test="${todo.processDefName != null && todo.processDefName != ''}">[${todo.processDefName }]</c:if><c:if test="${todo.processDefName == null || todo.processDefName == ''}">通知</c:if></td>
						...
						<td class="w60 gray">${fn:substring(todo.cTime, 0, 10)}</td>
						<td class="w60">
