...
var baseurl = '<%=request.getContextPath()%>';
	...
	//面门的调用方式
// 	function eventClick(){
// 		var usd = new UserSelectDialog('roleSelectCommonDialog',700,400,getPath2() + '/platform/user/bpmSelectUserAction/groupSelectCommon?isMultiple=true','地址簿');
// 		var buttons = [{
// 			text:'确定',
// 			id : 'processSubimt',
// 			iconCls : 'icon-submit',
...
function loadOrgTree(){
	 $('#orgTree').tree({
 		 ...
 		 onBeforeExpand:function(node,param){
 			var beforeExpandUrl = '/platform/user/bpmSelectUserAction/groupNoUser?';
			var para = node.attributes.para;
			if(typeof(para) == 'undefined'){
				para = '';
			}
			$('#orgTree').tree('options').url = getPath2() + beforeExpandUrl + 'appId='; //appId为应用ID，备用
	    }
 	 });
}
/**
...
function loadSelectedTable(){
	 $('#selectTargetDataGrid').datagrid({
			...
			columns:[[
				{field:'groupId',title:'群组ID',width:120,align:'left',hidden : true},
				{field:'groupName',title:'群组名称',width:150,align:'left'}
			]]
		});
}
//展开树
...
function eventRemoveButtonClick(){
	...
	for(var i = 0 ; i < datas.length; i++){
		var data = datas[i];
		var index = getRecordIndexInTargetDataGrid(data.groupId);
		$('#selectTargetDataGrid').datagrid('deleteRow', index);
		
		for(var i = 0; i < nodes.length; i++){
			if(getRecordIndexInTargetDataGrid(nodes[i].id) == -1 && nodes[i].id == data.groupId){
...
function getRecordIndexInTargetDataGrid(groupId){
	var datas = $('#selectTargetDataGrid').datagrid('getData');
	if(datas.total == 0){
		return -1;
	} else {
		for( var i = 0 ; i < datas.rows.length ; i++){
			if(groupId == datas.rows[i].groupId){
				return i;
			}
		}
	}
...
<div data-options="region:'west',title:'所有群组',split:true,collapsible:false"  style="width:300px;overflow: auto;">
	<ul id="orgTree"> </ul>  
</div>  
