...
function loadData(segmentIndex){
	...
	$.ajax({
        cache: false,
        type: "POST",
        url:'platform/cc/sysCodingRule/getSegmentValuesTableData.json',
        data : searchParam,
        dataType:"json",
        async: false,
        timeout:10000,
...
function reloadDatagrid(segmentIndex, searchSegmentName, searchSegmentValue){
	...
	$('#datagrid' + segmentIndex).datagrid('options').url = 'platform/cc/sysCodingRule/getSegmentValuesTableData.json';
...
function addSegmentValue(segmentIndex){
	var baseId = $('#baseId').val();
	var usd = new CommonDialog("addSegmentValueDialog","450","240","platform/cc/sysCodingRule/toAddSegmentValueJsp?baseId="+ baseId +"&segmentIndex=" + segmentIndex,"添加码值",true);
	var buttons = [{
		text:'提交',
		id : 'formSubimt',
		iconCls : '',
		...
		handler:function(){
			...
			$.ajax({
                cache: false,
                type: "POST",
                url:'platform/cc/sysCodingRule/addSegmentValue',
                dataType:"json",
...
function updateSegmentValue(segmentIndex){
	...
	var baseId = $('#baseId').val();
	var usd = new CommonDialog("updateSegmentValueDialog","450","240","platform/cc/sysCodingRule/toUpdateSegmentValueJsp?id="+ id +"&baseId="+ baseId +"&segmentIndex=" + segmentIndex,"编辑码值",true);
	var buttons = [{
		text:'提交',
		id : 'formSubimt',
		iconCls : '',
		...
		handler:function(){
			...
			$.ajax({
                cache: false,
                type: "POST",
                url:'platform/cc/sysCodingRule/updateSegmentValue',
                dataType:"json",
                data:frm.$('#formSegmentValue').serialize(),
                async: false,
                timeout:10000,
...
function deleteSegmentValue(segmentIndex){
	...
	$.messager.confirm('确认', '您确认删除吗?', function(r){
        if (r){
			...
			$.ajax({
				url : 'platform/cc/sysCodingRule/deleteSegmentValue',
				data : param,
				type : 'post',
				dataType : 'json',
				success : function(result) {
...
function exportSegmentValue(segmentIndex){
	var baseId = $('#baseId').val();
	$('#loadFileIframe').attr("src", getPath2() +'/platform/cc/sysCodingExcel/exportSegmentValueData?baseId=' + baseId + '&segmentIndex=' + segmentIndex);
}
...
function doImportSegmentValue(segmentIndex){
	if($('#importSegmentValueFile').val() != ''){
		...
		$('#formImportSegmentValue').form('submit', {
			url:'platform/cc/sysCodingExcel/importSegmentValues.json?baseId='+ baseId + '&segmentIndex=' + segmentIndex,
			dataType:'json',
			success: function(result){
...
function readImportErrorInfo(){
	var usd = new CommonDialog("errorInfoDialog","770","320","avicit/platform6/centralcontrol/syscodingrule/sysCodingUpdateSegmentValuesErrorInfo.jsp","导入数据错误信息",true,true,false);
	usd.show();
}
...
function downloadTemplate(segmentIndex){
	var baseId = $('#baseId').val();
	$('#loadFileIframe').attr("src", getPath2() +'/platform/cc/sysCodingExcel/downloadTemplate?baseId=' + baseId + '&segmentIndex=' + segmentIndex);
}
</script>
</head>
