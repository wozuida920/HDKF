...
<form id="formRuleSegment_${param.index}" method="post" style="padding: 15px 25px 0 0;">
	<table id="tableRuleSegment_${param.index}" class="form_commonTable">
		<tr>
			...
			<td colspan="3"><input id="segmentType_${param.index}" name="segmentType" class="easyui-combobox" required="true" data-options="editable:false,panelHeight:'auto',onShowPanel:comboboxOnShowPanel,onChange: function(newValue,oldValue){doChangeSegmentType(${param.index},newValue,oldValue)},valueField:'code',textField:'name',url:'platform/cc/sysCodingRule/getSegmentType.json'" /></td>
		</tr>
		<tr>
			<th>码段长度</th>
			...
			<th width="6%">后缀</th>
			<td><input id="segmentSuffix_${param.index}" name="segmentSuffix" class="easyui-validatebox" data-options="required:false,validType:'length[0,100]'"  /></td>
		</tr>
		<c:if test="${param.index != '1'}">
		<tr id="tdSegmentRelation_${param.index}" style="display: none">
			<th>分类码值关联<input type="hidden" id="segmentRelation_${param.index}" name="segmentRelation" /></th>
			<td colspan="7">
		...
		<tr id="tdComSegment_${param.index}" style="display: none">
			<th>通用编码</th>
			<td colspan="3"><input id="comSegmentId_${param.index}" name="comSegmentId" class="easyui-combobox" required="false" data-options="editable:false,panelHeight:100,onShowPanel:comboboxOnShowPanel,onChange: function(newValue,oldValue){doChangeComSegment(${param.index},newValue,oldValue)},valueField:'code',textField:'name',url:'platform/cc/sysCodingComSegment/getComSegmentData.json?appId=${param.appId}&isDisplaySelect=true'" /></td>
			<th></th>
			<td colspan="3"></td>
		</tr>
		...
		<tr id="tdCount_${param.index}" style="display: none">
			...
			<td colspan="3">
				<input id="format_${param.index}" name="format" class="easyui-combobox" data-options="editable:false,panelHeight: 100,onShowPanel:comboboxOnShowPanel,valueField:'code',textField:'name',url:'platform/cc/sysCodingRule/getCountFormatDate.json'" />
			</td>
