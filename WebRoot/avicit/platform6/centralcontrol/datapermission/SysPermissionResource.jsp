...
$(function(){
	...
	sysAppTree.setOnLoadSuccess(function(){
		 ...
		 sysMenu.setOnSelect(memuTreeOnClickRow)
	});
 ...
 * 菜单树单击事件
 */
function memuTreeOnClickRow(menutree,node) {
...
function showRole(rowIndex, field, value){
	var userSelect = new GridCommonSelector("role","datarole",rowIndex,"dept",{targetId:'accessRole'},"selectDialogCallBack",'{"appId":"'+_appId+'"}',null,null,false);//
	 var rows = $('#datarole').datagrid("getRows");
	 userSelect.init(rows[rowIndex]);
};
...
function checkSQL(sql){
	...
	$.ajax({
		 async: false, 
		 url:'platform/cc/permissionresource/check',
		 data : {sql:sql},
		 type : 'post',
		 dataType : 'json',
		 success : function(r){
...
function saveResource(){
	...
	**/
	var sqlvalue = document.getElementById("sql").value;
	 $.ajax({
		 url:'platform/cc/permissionresource/saveResource',
		 data : {data : data,
  ...
  if(rows.length > 0){
	  $.messager.confirm('请确认','您确定要删除当前所选的数据？',function(b){
		 if(b){
			 ...
			 $.ajax({
				 url:'platform/cc/permissionresource/deleteResource',
				 data:{
					 ids : ids.join(',')
				 },
				 type : 'post',
	...
	if(node){
		if(node._parentId ==='-1'){
		 ...
		 $.messager.confirm('请确认','您确定要刷新当前权限资源？',function(b){
			 if(b){
				 $.ajax({
					 url:'platform/cc/permissiontree/refreshResource/'+_appId+".json",
					 data:{
						 id:node.id
					 },
					 type:'post',
					 ...
					 success:function(r){
						 if(r.flag ==="success"){
							 ...
							 $.messager.alert('提示', '操作失败，'+r.e, 'warning');
						 }
...
function reloadResource(nodeinf,param){
	var node = nodeinf || sysMenu._selectNode;
	var vparam =param||{};
	$.ajax({
		cache : true,
		type : "POST",
		url : 'platform/cc/permissiontree/listResource.json',
		dataType : "json",
		data : {
			id : node.id,
			param :vparam
...
function reloadAccess(rowData){
	...
	$.ajax({
		cache : true,
		type : "POST",
		url : 'platform/cc/permissiontree/listAccess.json?id'+ row.id,
		dataType : "json",
		data : {
			id : row.id
		},
...
function openDeptLeaderDialog(){
	var usd = new CommonDialog("deptLeaderDialog","600","300","avicit/platform6/centralcontrol/datapermission/deptleader.jsp","选择部门领导",false,false,false);
	usd.show();
};
