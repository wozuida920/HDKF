...
function loadSelectTabInfo(title){
	...
	if(title=="部门基本信息"){
		...
		if(currTreeNode){
			...
			if(oldUrl==""){
				var newUrl=path+"/avicit/platform6/centralcontrol/sysdept/sysuserList.jsp?id="+currTreeNode.id+"&type="+currTreeNode.attributes.type+"&name="+encodeURI(currTreeNode.text);
			...
			var oldUrl=$("#iframeUserList").attr("src");
			if(oldUrl==""){
				var newUrl=path+"/avicit/platform6/centralcontrol/sysdept/sysuserList.jsp";
				$("#iframeUserList").attr("src",newUrl);
			}
		}
	 ...
	 $.messager.confirm('请确认','您确定要删除当前所选的数据？',function(b){
		if(b){
		 $.ajax({
		        cache: false,
		        type: "POST",
		        url:'platform/cc/sysdept/deleteDept.json',
		        dataType:"json",
...
<div data-options="region:'west',split:true,title:''" style="width:250px;padding:0px;">
	...
	<ul id="deptTree" class="easyui-tree" data-options="
			...
			url:'platform/cc/sysdept/getChildOrgDeptById.json',
...
<div data-options="region:'center',split:true,title:''" style="padding:0px;">
	<div id="deptUserTab" class="easyui-tabs" data-options="fit:true,plain:true,tabPosition:'top',toolPosition:'right',onSelect:onTabSelect" style="padding:1px;"  >
		<div id="tabDept" iconCls="icon-dept" title="部门基本信息">
			<div id="toolbar" class="datagrid-toolbar">
				<sec:accesscontrollist   hasPermission="3" domainObject="ccsysdept_toolbar_btAddDept" permissionDes="添加">
					<a id="btAddDept"  class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="addDept();" href="javascript:void(0);">添加</a>
				</sec:accesscontrollist>
				<sec:accesscontrollist   hasPermission="3" domainObject="ccsysdept_toolbar_btEditDept" permissionDes="编辑">
				...
				<sec:accesscontrollist   hasPermission="3" domainObject="ccsysdept_toolbar_btSaveDept" permissionDes="保存">
				...
				<sec:accesscontrollist   hasPermission="3" domainObject="ccsysdept_toolbar_btDeleteDept" permissionDes="删除">
			...
			<form id="deptEditForm" name="deptEditForm" method="post">
				...
				<tr>
					<sec:accesscontrollist hasPermission="3" domainObject="pmproject_pmProjectForm_editForm_WORK_CALENDAR_ID" permissionDes="工作日历">
						<th>工作日历：</th>
						<td><input class="easyui-validatebox" disabled="disabled" type="text" name="WORK_CALENDAR_ID" id="WORK_CALENDAR_ID" ></input></td>
					</sec:accesscontrollist>
