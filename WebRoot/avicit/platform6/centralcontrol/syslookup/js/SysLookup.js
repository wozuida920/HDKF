...
SysLookup.prototype.loadById=function(pid){
	this.endEditing();
	(this._datagrid&&this._datagrid.datagrid({url : this.getUrl() + "/allSysLookup/" + pid}).datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections'))||this.init(pid);
};
...
//删除
SysLookup.prototype.del=function(pid){
	...
	var l =rows.length;
	  ...
	  $.messager.confirm('请确认','您确定要删除当前所选的数据？',function(b){
		 if(b){
			 ...
			 $.ajax({
				 url:'platform/cc/syslookup/operation/lookup/delete/'+pid,
...
SysLookup.prototype.openLanguageForm=function(){
	...
	this.cData = new CommonDialog("chooseL","700","400",this.getUrl()+'/-11/chooseLanguage/2/'+rows[0].id,"多语言设置",false,true,false);
	this.cData.show();
};
//重载数据
