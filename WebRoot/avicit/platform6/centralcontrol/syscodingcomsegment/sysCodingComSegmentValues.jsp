...
function addSegmentValue(){
	...
	var usd = new CommonDialog("addSegmentValueDialog","380","225","platform/cc/sysCodingComSegment/toAddComSegmentValueJsp?segmentId="+ segmentId,"添加码值",true);
	var buttons = [{
		text:'提交',
		id : 'formSubimt',
		iconCls : '',
		...
		handler:function(){
			...
			$.ajax({
                cache:false,
                type: "POST",
                url:'platform/cc/sysCodingComSegment/addSegmentValue',
                dataType:"json",
...
function updateSegmentValue(){
	...
	var id = checked[0].id;
	var segmentId = $('#segmentId').val();
	var usd = new CommonDialog("updateSegmentValueDialog","380","225","platform/cc/sysCodingComSegment/toUpdateComSegmentValueJsp?id="+ id +"&segmentId="+ segmentId,"编辑码值",true);
	var buttons = [{
		text:'提交',
		id : 'formSubimt',
		iconCls : '',
		...
		handler:function(){
			...
			$.ajax({
                cache:false,
                type: "POST",
                url:'platform/cc/sysCodingComSegment/updateSegmentValue',
                dataType:"json",
                data:frm.$('#formSegmentValue').serialize(),
                async: false,
                timeout:10000,
...
function deleteSegmentValue(){
	...
	$.messager.confirm('确认', '您确认删除吗?', function(r){
        if (r){
			...
			$.ajax({
				url : 'platform/cc/sysCodingComSegment/deleteSegmentValue',
				data : param,
				type : 'post',
				dataType : 'json',
				success : function(result) {
...
<div region="center" border="false">
	<table id="datagrid1" class="easyui-datagrid" 
		data-options="
			...
			selectOnCheck: true,
			method: 'post',
			url:'platform/cc/sysCodingComSegment/getComSegmentValuesListData.json?segmentId=${param.id }'
		">
		<thead>
			<tr>
