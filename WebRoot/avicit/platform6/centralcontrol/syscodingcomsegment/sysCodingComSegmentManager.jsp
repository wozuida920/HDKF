...
function addComSegment(){
	var usd = new CommonDialog("addComSegmentDialog","520","245","avicit/platform6/centralcontrol/syscodingcomsegment/sysCodingComSegmentAdd.jsp","添加通用码段",true);
	var buttons = [{
		text:'保存',
		id : 'formSubimt',
		iconCls : '',
		...
		handler:function(){
			...
			$.ajax({
				url : 'platform/cc/sysCodingComSegment/addComSegment',
				data : param,
				type : 'post',
				dataType : 'json',
...
function updateComSegment(){
	...
	var usd = new CommonDialog("updateComSegmentDialog","520","245","platform/cc/sysCodingComSegment/toUpdateComSegmentJsp.jsp?id="+id,"编辑通用码段",true);
	var buttons = [{
		text:'保存',
		id : 'formSubimt',
		iconCls : '',
		...
		handler:function(){
			...
			$.ajax({
				url : 'platform/cc/sysCodingComSegment/updateComSegment',
				data : param,
				type : 'post',
				dataType : 'json',
				success : function(result) {
...
function deleteComSegment(){
	...
	$.messager.confirm('确认', '您确认删除吗?', function(r){
        if (r){
			...
			$.ajax({
				url : 'platform/cc/sysCodingComSegment/deleteComSegment',
				data : param,
				type : 'post',
				dataType : 'json',
				success : function(result) {
...
function updateComSegmentValues(){
	...
	var usd = new CommonDialog("updateComSegmentValuesDialog","600","320","avicit/platform6/centralcontrol/syscodingcomsegment/sysCodingComSegmentValues.jsp?id="+id,"通用码段码值管理",true,true,false);
	usd.show();
}
...
<div region="center" border="false">
	<table id="datagrid1" class="easyui-datagrid" 
			data-options="
				...
				autoRowHeight: true,
				method: 'post',
				url:'platform/cc/sysCodingComSegment/getComSegmentListData.json?appId=${param.appId}',
				idField: 'id',
				toolbar:'#toolbar',
				singleSelect: false,
				checkOnSelect: true,
	...
	<div id="toolbar" class="datagrid-toolbar" style="height:auto;display: block;">
		<div style="padding:0 0 0 2px;">
			<table width="100%">
			   <tr>
			   	<td width="60px" align="right">码段名称</td>
			   	<td width="150px"><input class="easyui-validatebox" id="searchName" name="searchName" style="width: 150px" value=""/></td>
				<td width="100px">
