...
//	var dataGridHeight = $(".easyui-layout").height()-73;
	$('#roleList').datagrid({
		fit: true,
	    url:'platform/cc/sysaccesscontrol/getSysAccesscontrolData/R.json',
		idField:'ID',
		//width: '100%',
		singleSelect: true,
	    nowrap: false,
...
function loadGroupList(){
	$('#groupList').datagrid({
		fit: true,
		url:'platform/cc/sysaccesscontrol/getSysAccesscontrolData/G.json',
		idField:'ID',
		singleSelect: true,
	    nowrap: false,
	    striped: true,
...
function loadPositionList(){
	...
	$('#positionList').datagrid({
		fit: true,
		url:'platform/cc/sysaccesscontrol/getSysAccesscontrolData/P.json',
		idField:'ID',
		//width: '100%',
	    nowrap: false,
	    striped: true,
...
function loadDeptTree(){
	    ...
	    $('#orgTree_dept').datagrid({
			fit: true,
			url:'platform/cc/sysaccesscontrol/getSysAccesscontrolData/D.json',
			idField:'ID',
			//width: '100%',
		    nowrap: false,
		    striped: true,
...
function comReadAuthFormat(value, rowData, rowIndex) {
	if("1" == value)   
		return "<img src='static/images/platform/sysmenuaccesscontrol/ok.png' title='允许访问' alt='允许访问' >";
	if("0" == value) 
		return "<img src='static/images/platform/sysmenuaccesscontrol/no.gif' title='禁止访问' alt='禁止访问' >";
	
	return "<img src='static/images/platform/sysmenuaccesscontrol/untitled.png' title='未设置权限' alt='未设置权限' >";	
}
//编辑权限formatter
function comWriteAuthFormat(value, rowData, rowIndex) {
	if("1" == value)   
		return "<img src='static/images/platform/sysmenuaccesscontrol/ok.png' title='允许编辑' alt='允许编辑' >";
	if("0" == value) 
		return "<img src='static/images/platform/sysmenuaccesscontrol/no.gif' title='禁止编辑' alt='禁止编辑' >";
			...
			.tabs(
					{
						onSelect : function(title, index) {
							...
							if (index == 1) {
								...
								$("#dgUser").datagrid("options").url='platform/cc/sysaccesscontrol/getSysAccesscontrolData/U.json';
		...
		$.messager.confirm('请确认','您确定要删除当前所选的数据？',
			function(b){
				if(b){
					...
					$.ajax({
						url : 'platform/cc/sysaccesscontrol/deleteSysAccesscontrol.json',
		...
		$.messager.confirm('请确认','您确定要修改当前所选的数据权限？',
			function(b){
				if(b){
					...
					$.ajax({
						url : 'platform/cc/sysaccesscontrol/updateComponentResourceAuth.json',
...
function selectUserDialogCallBack(data){
	...
	for(;l--;){
	 ...
	 $.ajax({
		 url:'platform/cc/sysaccesscontrol/insertComponentResourceAuth/U/'+currentMenuId+'/'+_APPID+'.json',
		 data:	JSON.stringify(ids),
		 contentType : 'application/json',
		 type : 'post',
		 dataType : 'json',
...
function insertRole(){
	//_APPID为主页面定义的全局变量
	var path="platform/cc/menuaccesscontrol/toAddRoleJsp?appId="+_APPID;
...
function insertGroup(){
	var path="platform/cc/menuaccesscontrol/toAddGroupJsp";
...
function insertPosition(){
	var path="platform/cc/menuaccesscontrol/toAddPositionJsp";
...
function selectDeptDialogCallBack(data){
	...
	$.ajax({
		 url:'platform/cc/sysaccesscontrol/insertComponentResourceAuth/D/'+currentMenuId+'/'+_APPID+'.json',
		 data:	JSON.stringify(ids),
		 contentType : 'application/json',
		 type : 'post',
		 dataType : 'json',
