...
var path="<%=ViewUtil.getRequestPath(request)%>";
	...
	function loadPositionInfo() {
		...
		$("#dgPosition").datagrid("options").url = "platform/cc/sysposition/getSysPositionVoList.json";
		$('#dgPosition').datagrid("reload", {});
		$("#dgPosition").datagrid('uncheckAll').datagrid('unselectAll')
	...
	function initComboData() {
		$
				.ajax({
					url : 'platform/syslookuptype/getLookUpCode/PLATFORM_USER_STATUS.json',
					type : 'get',
					dataType : 'json',
					success : function(r) {
						r && (validComboData = r);
	...
	function savePosition() {
		...
		if (rows.length > 0) {
			$.ajax({
				url : 'platform/cc/sysposition/saveSysPositionVo.json',
				data : {
					datas : data
				},
				type : 'post',
		...
		if (l > 0) {
			$.messager.confirm('请确认', '您确定要删除当前所选的数据？', function(b) {
				if (b) {
					...
					$.ajax({
						url : 'platform/cc/sysposition/deleteSysPositionVo',
	...
	function loadUserInfo(positionId) {
		...
		if (positionId == null) {
			...
			$("#dgUser").datagrid('uncheckAll').datagrid('unselectAll')
					.datagrid('clearSelections');
		} else {
			$("#dgUser").datagrid("options").url = "platform/cc/sysposition/getUserListByPage/null";
	...
	function changePosition() {
		...
		var usd = new CommonDialog("userAddDialog", "800", "500",
				"platform/cc/sysposition/changePosition/" + positionId, "批量换岗");
		var buttons = [ {
			text : '添加',
			id : 'formSubimt',
			//iconCls : 'icon-submit',
			handler : function() {
				...
				$.ajax({
					cache : false,
					type : "POST",
					url : 'platform/cc/sysposition/doChangeUserPosition',
					dataType : "json",
					data : {
						positionId : positionId,
						sysUserIds : data.join(',')
					},
					error : function(request) {
						$.messager.alert('提示', '操作失败，服务请求状态：' + request.status
	...
	/* 
	 ...
	 $.ajax({
	 url : 'platform/cc/sysrole/deleteUserRole',
	 data : {
	...
	function showSearchUser() {
		$('#filter_EQ_u_status').combobox('loadData', [ {
			lookupCode : '',
			lookupName : '所有'
		} ].concat(validComboData));
	...
	function loadSearchUserInfo(positionId, name, loginName, status) {
		...
		$("#dgUser").datagrid('uncheckAll').datagrid('unselectAll').datagrid(
				'clearSelections');
	...
	function importPosition() {
		var imp = new CommonDialog(
				...
				"700",
				"400",
				'platform/excelImportController/excelimport/importPosition/xls',
						...
						function(r) {
							if (r) {
								...
								var myParams = {
									...
									unContainFields : 'ID',
									fileName : '平台岗位导出数据'
											+ new Date().toString()//导出的Excel文件名
								};
								var url = "platform/cc/sysposition/exportClient";
								var ep = new exportData("xlsExport",
										"xlsExport", myParams, url);
								ep.excuteExport();
							}
						});
	};
				...
				.confirm(
						...
						function(r) {
							if (r) {
								...
								var url = "platform/cc/sysposition/exportServer";

								var ep = new exportData("xlsExport",
										"xlsExport", expSearchParams, url);
								ep.excuteExport();
							}
						});
	};
...
<body class="easyui-layout" fit="true">
	...
	<div data-options="region:'north',split:true,title:''"
		...
		<div id="toolbarPosition" class="datagrid-toolbar">
			<a id="btAddGroup" class="easyui-linkbutton" iconCls="icon-add"
				plain="true" onclick="showAddPosition();" href="javascript:void(0);">添加</a>
			<a id="btSaveGroup" class="easyui-linkbutton" iconCls="icon-save"
				plain="true" onclick="savePosition();" href="javascript:void(0);">保存</a>
			<a id="btChangePosition" class="easyui-linkbutton"
				iconCls="icon-setting" plain="true" onclick="changePosition();"
				href="javascript:void(0);">批量换岗</a> <a id="btImportPosition"
				class="easyui-linkbutton" iconCls="icon-import" plain="true"
				onclick="importPosition();" href="javascript:void(0);">导入岗位</a>
			<td><a class="easyui-menubutton"
				data-options="menu:'#export',iconCls:'icon-export'"
				href="javascript:void(0);">导出岗位</a></td> <a id="btDeleteGroup"
				class="easyui-linkbutton" iconCls="icon-remove" plain="true"
				onclick="deletePosition();" href="javascript:void(0);">删除</a> <a
				id="btSearchGroup" class="easyui-linkbutton" iconCls="icon-search"
				plain="true" onclick="showSearchPosition();"
				href="javascript:void(0);">查询</a>
		</div>

		<div id="export" style="width:150px;">
			<div data-options="iconCls:'icon-excel'"
				onclick="exportClientData();">Excel导出(客户端)</div>
			<div data-options="iconCls:'icon-excel'"
				onclick="exportServerData();">Excel导出(服务器端)</div>
		</div>
		...
		<table id="dgPosition" class="easyui-datagrid"
			data-options=" 
					...
					url: 'platform/cc/sysposition/getSysPositionVoList.json',
			...
			<thead>
				<tr>
					<th data-options="field:'id', halign:'center',checkbox:true"
						width="220">id</th>
					<th
						data-options="field:'positionCode', halign:'center',align:'left',required:true"
						editor="{type:'validatebox',options:{required:true}}" width="220"><span
						class="red-start">*</span>岗位代码</th>
					<th
						data-options="field:'positionName', halign:'center',align:'left',required:true"
						editor="{type:'validatebox',options:{required:true}}" width="220"><span
						class="red-start">*</span>岗位名称</th>
					<th
						data-options="field:'positionDesc', halign:'center',align:'left'"
						editor="{type:'text'}" width="220">描述</th>
					<th data-options="field:'orderBy',halign:'center',align:'left'"
	...
	<div id="searchPositionDialog" class="easyui-dialog"
			...
			<table>
				<tr>
					...
					<td><input type='text' class="easyui-validatebox"
						name="filter_LIKE_positionCode" id="filter_LIKE_positionCode" /></td>
					<td>岗位名称:</td>
					<td><input type='text' class="easyui-validatebox"
						name="filter_LIKE_positionName" id="filter_LIKE_positionName" /></td>
				</tr>
			</table>
		</form>
		<div id="searchBtns">
			<a class="easyui-linkbutton" plain="false"
				onclick="searchPosition();" href="javascript:void(0);">查询</a> <a
				class="easyui-linkbutton" plain="false" onclick="clearPosition();"
				href="javascript:void(0);">清空</a> <a class="easyui-linkbutton"
				plain="false" onclick="hideSearchPosition();"
	...
	<div data-options="region:'center',split:true,title:''"
		...
		<div id="toolbarUser" class="datagrid-toolbar">
			<a id="btSearchUser" class="easyui-linkbutton" iconCls="icon-search"
				plain="true" onclick="showSearchUser();" href="javascript:void(0);">查询用户</a>
		</div>
		...
		<table id="dgUser" class="easyui-datagrid"
			...
			<thead>
				<tr>
					<th
						data-options="field:'no',required:true,halign:'center',align:'left'"
						editor="{type:'text'}" width="220">用户编号</th>
					<th
						data-options="field:'name',required:true,halign:'center',align:'left'"
						editor="{type:'text'}" width="220">用户名</th>
					<th
						data-options="field:'nameEn',required:true,halign:'center',align:'left'"
						editor="{type:'text'}" width="220">英文名</th>
					<th data-options="field:'loginName',halign:'center',align:'left'"
						editor="{type:'text'}" width="220">登录名</th>
					<th data-options="field:'deptCode',halign:'center',align:'left'"
						editor="{type:'text'}" width="220">部门编号</th>
					<th data-options="field:'deptName',halign:'center',align:'left'"
						editor="{type:'text'}" width="220">所属部门</th>
					<th
						data-options="field:'status',halign:'center',align:'left', formatter: formatcombobox"
						editor="{type:'combobox',options:{required:true,panelHeight:'auto',editable:false,valueField:'lookupCode',textField:'lookupName'}}"
						width="220">状态</th>
					<th data-options="field:'remark',halign:'center',align:'left'"
						editor="{type:'text'}" width="220">描述</th>
	...
	<div id="searchUserDialog" class="easyui-dialog"
		data-options="iconCls:'icon-search',resizable:true,modal:false,buttons:'#searchUserBtns'"
		...
		<form id="searchUserForm">
			<table>
				...
				<tr>
					<td>用户名:</td>
					<td><input type='text' class='easyui-validatebox'
						name="filter_LIKE_name" id="filter_LIKE_u_name" /></td>
					<td>登录名:</td>
					<td><input type='text' class='easyui-validatebox'
						name="filter_LIKE_loginName" id="filter_LIKE_u_loginName" /></td>
				</tr>
				<tr style="height:5px;">
				</tr>
				<tr>
					<td>状态:</td>
					<td><input id="filter_EQ_u_status" class="easyui-combobox"
						name="filter_EQ_u_status"
						data-options="panelHeight:'auto', editable:false,valueField:'lookupCode',textField:'lookupName', data: [{lookupCode:'1', lookupName: 'aaa'}]" />
					</td>
		...
		<div id="searchUserBtns">
			<a class="easyui-linkbutton" plain="false" onclick="searchUser();"
				href="javascript:void(0);">查询</a> <a class="easyui-linkbutton"
				plain="false" onclick="clearUser();" href="javascript:void(0);">清空</a>
			<a class="easyui-linkbutton" plain="false"
				onclick="hideSearchUser();" href="javascript:void(0);">返回</a>
		</div>
	</div>
