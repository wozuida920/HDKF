 ...
 */
var _selfApp={};
function SysApplication(id,datagrid,searchForm,url,searchD){
...
SysApplication.prototype.save=function(){
	...
	if(rows.length > 0){
		 _self._indexEditing=-1;
		 $.ajax({
			 url: 'platform/cc/sysapp/save.json',
			 data : {datas : data},
			 type : 'post',
			 dataType : 'json',
			 success : function(r){
	...
	var l =rows.length;
	  ...
	  $.messager.confirm('请确认','您确定要删除当前所选的数据？',function(b){
		 if(b){
			 ...
			 $.ajax({
				 url:'platform/cc/sysapp/delete.json',
				 data:{
					 ids : ids.join(',')
				 },
				 type : 'post',
...
SysApplication.prototype.formatInit=function(value,row,index){
	if(row.id === this._id){
		return '<span style="color:green;">已经初始化！</span>';
	}else{
		return '<span style="color:red;cursor:pointer;" onclick="_selfApp.InitM(\''+row.id+'\',\''+row.applicationCode+'\');">初始化</span>';
	}
};
SysApplication.prototype.openSearch=function(){
	$(this._searchDiaId).dialog('open',true);
};
SysApplication.prototype.hideSearch=function(){
	$(this._searchDiaId).dialog('close',true);
};
SysApplication.prototype.InitM=function(id,code){
	if(!id){
		alert("请保存记录后，再初始化！");
	}
	var _self = this;
	$.ajax({
		 url:_self._url+"/initM/"+id+"/"+code,
		 type : 'post',
		 dataType : 'json',
		 async:false,
		 success : function(r){
