...
function mySelectRole(){
	var userSelector = new CommonSelector("role","roleSelectCommonDialog","ruleId","ruleName",null,null,null,null,null,null,600,400);
	...
	if(document.getElementById("sysUserPhoto").value != ''){
		if(checkfiletype('sysUserPhoto')){
			...
			$('#uploadForm').form('submit', {
				url: 'platform/cc/sysuserphoto/upload/headerphoto?userId='+userId,
				success: function(){
					$.messager.progress('close');	// 如果提交成功则隐藏进度条
					$.messager.alert('提示','头像文件上传成功!','info',function(r){
						$('#addUserPhotoDialog').dialog('close',true);
						document.getElementById("sysUserHeadPhotoImg").src="platform/cc/sysuserphoto/upload/headerphoto?sysUserId="+userId+"&o=" + Math.random();
					});
				}
			});
			return;
	...
	if(document.getElementById("sysUserSign").value != ''){
		if(checkfiletype('sysUserSign')){
			...
			$('#uploadPersionSignForm').form('submit', {
				url: 'platform/cc/sysuserphoto/upload/signphoto?userId='+userId,
				success: function(){
					$.messager.progress('close');	// 如果提交成功则隐藏进度条
					$.messager.alert('提示','签名文件上传成功!','info',function(r){
						$('#addUserSignDialog').dialog('close',true);
						document.getElementById("sysUserSignPhotoImg").src="platform/cc/sysuserphoto/upload/signphoto?sysUserId="+userId+"&o=" + Math.random();
					});
				}
			});
			return;
		...
		$.messager.alert('警告','请选择要上传的签名文件!','warning');
		return;
	}
}
...
function checkUserCode(myObj,userId){
	$(myObj).validatebox({ 
	    validType : "unique['platform/cc/sysuser/checkUserCode?sysUserId="+userId+"','userCode']"
	});
...
function checkLoginName(myObj,userId){
	$(myObj).validatebox({ 
	    validType : "unique['platform/cc/sysuser/checkLoginName?sysUserId="+userId+"','loginName']"
	});
};
//进入集团统一代码框验证规则
...
function checkUnitCode(myObj,userId){
	$(myObj).validatebox({ 
	    validType : "unique['platform/cc/sysuser/checkUnitCode?sysUserId="+userId+"','unitCode']"
	});
};