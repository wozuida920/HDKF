...
function changeTabs(title,index,userId){
	...
	if(index == 1){
		...
		if(!dgMapUserRole){
			$.ajax({
				url: 'platform/cc/sysuser/getRoleCode.json',
				data :{},
				type :'POST',
				dataType :'json',
				success : function(r){
...
function saveForm(){
	...
	$.ajax({
		url : 'platform/cc/sysuser/toSaveUser/2',
		data : {
...
function saveMapDept(id){
	...
	if(rows.length > 0){
		 $.ajax({
			 url:'platform/cc/sysuser/saveMapDept',
		...
		$.messager.confirm('请确认','您确定要删除当前所选的数据？',
			function(b){
				if(b){
					...
					$.ajax({
						url : 'platform/cc/sysuser/deleteMapDept',
						data : {
							ids : ids.join(',')
						},
						type : 'post',
...
function saveMapUserRole(userId){
	...
	if(count > 0){
		/* for (;count--;) {
		 ...
		 $.ajax({
			 url:'platform/cc/sysuser/saveMapRole',
		...
		$.messager.confirm('请确认','您确定要删除当前所选的数据？',
			function(b){
				if(b){
					...
					$.ajax({
						url : 'platform/cc/sysuser/deleteMapRole',
						data : {
