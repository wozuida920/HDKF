...
function saveAddForm(){
	...
	dataVo = JSON.stringify(dataJson);
	$.ajax({
		url : 'platform/cc/sysuser/toSaveUser/3',
		data : {
			datas : JSON.stringify(serializeObject($('#formAddBaseInfo')))
		},
		type : 'post',
...
//				doc.getElementById('deptId').value='';
				...
				doc.getElementById('sysUserHeadPhotoImg').src='static/images/platform/sysuser/userPhoto.gif';
				doc.getElementById('sysUserSignPhotoImg').src='static/images/platform/sysuser/userSign.gif';
				parent.flushData();
			}else {
				var msg ='操作失败！';
				if(result.error){
