...
<head>
	...
	<script type="text/javascript" charset="utf-8">
		...
		$(function() {
			...
			$.ajax({
				url: 'platform/syslookuptype/getLookUpCode/PLATFORM_SYS_USER_RELATION_TYPE.json',
				type :'get',
				dataType :'json',
				success : function(r){
					if(r){
		...
		function userRelationSearchData() {
			...
			$("#userRealtionDataGrid").datagrid("options").url ="platform/cc/userrelation/getSysUserRelationVo.json";
		...
		function saveData()
		{
			...
			{
				 $.ajax({
					 url:'platform/cc/userrelation/saveOrUpdateSysUserRelation',
					 data : {datas : data},
					 type : 'post',
					 async : false,
					 dataType : 'json',
			...
			if (rows.length > 0) {
				$.messager.confirm('请确认', '您确定要删除当前所选的数据？', function(b) {
					if (b) {
						...
						$.ajax({
							url : 'platform/cc/userrelation/deleteUserRelation',
							data : {
								ids : ids.join(",")
							},
							type : 'post',
			...
			if (rows.length > 0) {
				...
				$.messager.confirm('请确认', msg , function(b) {
					if (b) {
						...
						$.ajax({
							url : 'platform/cc/userrelation/changeValidFlag',
							data : {
								ids : ids.join(","),
								flag : flag
							},
 ...
 <div data-options="region:'center',split:true,border:false" style="padding:0px;overflow:hidden;">
	...
	<table id="userRealtionDataGrid" class="easyui-datagrid"
		data-options=" 
			...
			striped:true,
			url:'platform/cc/userrelation/getSysUserRelationVo.json',
		...
		<thead>
			<tr>
				...
				<th data-options="field:'user1Name', halign:'center',align:'left'" editor="{type:'validatebox',options:{required:true}}"  width="220"><span class="red-start">*</span>用户1</th>
				...
				<th data-options="field:'user2Name', halign:'center',align:'left'" editor="{type:'validatebox',options:{required:true}}"   width="220"><span class="red-start">*</span>用户2</th>
	...
	<div id="userRelationSearchDialog" class="easyui-dialog" data-options="iconCls:'icon-search',modal:false,buttons:'#userRelationsearchBtns'" 
			style="width: 480px;height:200px;visible: hidden;" title="查询条件">
		<form id="userRelationSearchForm">
    		...
    		<table>
    			...
    			<tr>
    				...
    				<td>用户1:</td><td><input id="filter_LIKE_user1Name" type='text' name="filter_LIKE_user1Name" style="width:150px;" readonly="readonly"/></td>
    				
    				<td>用户2:</td><td><input id="filter_LIKE_user2Name" type='text' name="filter_LIKE_user2Name" style="width:150px;" readonly="readonly"/></td>
    			</tr>
    			<tr>
    				<td>用户关系:</td>
    				<td colspan="2">
