	...
	var buttons = [{
		...
		handler:function(){
			 ...
			 $.ajax({
				...
				url : "platform/bpm/bpmPublishAction/addProcessForm",
				dataType : "json",
				success : function(result) {
					addProcessFormBack(result);
				}
			});
		}
	}];
	...
	var buttons = [{
		...
		handler:function(){
			 ...
			 $.ajax({
				...
				url : "platform/bpm/bpmPublishAction/updateProcessForm",
				dataType : "json",
				success : function(result) {
					editProcessFormBack(result);
				}
			});
		}
	}];
	...
	<div id="toolbar" class="datagrid-toolbar" style="height:auto;display: block;">
			...
			<table class="tableForm" id="searchForm" width='100%'>
				<tr>
					...
					<td ><span>电子表单规则：表名即是表单代码，url是"platform/eform/cbbCRUDClient/bpm?tableId="+表名</span></td>
				</tr>
			</table>
