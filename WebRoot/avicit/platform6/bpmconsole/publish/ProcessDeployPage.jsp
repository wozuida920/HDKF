...
function selectForm(type){
	...
	usd = new UserSelectDialog("lcbdxzk", "700", "500",encodeURI(url), "流程表单选择框");
	var buttons = [{
		text : '确定',
		id : 'processSubimt',
		//iconCls : 'icon-submit',
...
function activityProcessForm(actValue){
	...
	if(actValue!=null && actValue!='' && actFormCode!='请选择...' && actFormCode!=''){
		var tempUrl = 'platform/bpm/bpmPublishAction/getProcessDeployForm.json?type=activity&pdId=' + processDefId + '&pdActivityName=' + actValue+'&actFormCode='+actFormCode;
	...
	$('#btnSubmitActivity').bind('click',
			function() {
				$('#activityForm').form('submit', {   
		   			url:'platform/bpm/bpmPublishAction/saveProcessDeployForm?type=activity&pdId=' + processDefId+'&formId=' + $('#pdGlobalFormName').combobox('getValue')+'&formCode=' + $('#pdGlobalFormName').combobox('getText'),   
		    		onSubmit: function(){   
				...
				if(processDefId!=null && processDefId != '' && processDefId != 'undefined'){
					$.messager.confirm("操作提示", "您确认要删除吗?", function(data) {
						if (data) {
							...
							ajaxRequest(
									"POST",
									"type=global&pdId=" + processDefId+"&formIds="+formIds,
				...
				if(tempId!=null && tempId != '' && tempId != 'undefined'){
					$.messager.confirm("操作提示", "您确认要删除吗?", function(data) {
						if (data) {
							...
							ajaxRequest(
									"POST",
									"type=activity&pdId=" + tempId+"&formIds=" + $("#pdGlobalFormName").combobox("getValue"),
									"platform/bpm/bpmPublishAction/deleteProcessDeployForm",
									"json", "backChecked");
							document.getElementById("formId").value="";
							document.getElementById("formUrl").value="";
        ...
        <div class="easyui-layout" data-options="fit:true">   
						...
						<div id="toolbarGlobalFormSetting" class="datagrid-toolbar">
							<table>
								<tr>
									...
									<td><a class="easyui-linkbutton" plain="true" 
										iconCls="icon-reload"  id="btnRefresh" href="javascript:void(0);">刷新表单缓存</a>
									</td>
								</tr>
							</table>
						</div>
						<table id="dgGlobalFormSetting" 
							...
							<thead>
								<tr>
									...
									<th data-options="field:'formUrl', halign:'center'"  width="220">表单URL</th>
            ...
            <form id="activityForm" method="post" action=""
            	...
            	<div style="margin:8px;">
					当前全局表单：<input class="easyui-combobox" id="pdGlobalFormName"  name="pdGlobalFormName" value='请选择...' valueField="formId" editable="false" required="true"  textField="formCode" />
					&nbsp;&nbsp;&nbsp;&nbsp;
					当前节点名称：<input class="easyui-combobox" id="pdActivityName" name="pdActivityName" value='请选择...' valueField="actId"  editable="false" required="true"  textField="alias" />
					</div>
							
								<fieldset style="height:auto;">
									<legend>当前节点表单配置</legend>
									<input type="hidden" name="pdId" id="pdId" /> <input type="hidden"
										name="id" id="id" /> <input type="hidden" name="formId"
										id="formId" />
									<table>
										<tr>
