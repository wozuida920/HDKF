...
function loadProcessPublish(){
	var dataGridHeight = $(".easyui-layout").height()-$("#toolbar").height();//85;
	$('#processlist').datagrid({
		...
		columns:[[
			...
			{field:'processDefId',title:'流程定义ID',width:100,align:'left'},
...
function setProcessStartRight(){
	...
	var url = getPath2()+"/avicit/platform6/bpmconsole/publish/ProcessStartRight.jsp?deployment="+deployment;
	top.addTab("流程启动权限",url,"static/images/platform/index/images/icons.gif","ProcessStartRight"," -0px -120px");
}
/**
 * 设置流程的意见
 */
function setIdeaStyle(){
	...
	var url = getPath2()+"/avicit/platform6/bpmconsole/ideastyle/IdeaStyleManager.jsp?pdId="+pdId;
	top.addTab("流程意见设置",url,"static/images/platform/index/images/icons.gif","ProcessIdeaStyle"," -0px -120px");
}
...
function lookUpTrackImage() {
	...
	var url = getPath2() + "/platform/bpm/bpmPublishAction/getProcessImageTrack.gif";
...
function publishProcessTemplate() {
	...
	var url = getPath2() + "/avicit/platform6/bpmconsole/publish/ProcessPublishPage.jsp";
...
function reloadProcessTemplate(){
	...
	if (data == null || data=='') {
		...
		var deploymentId = data.deployId;
		$.messager.confirm("操作提示", "您确定要加载选择的流程吗?", function(data) {
			if (data) {
				easyuiMask();
...
//加载全部
function reloadProcessTemplateAll(){
	$.messager.confirm("操作提示", "您确定要加载所有流程吗?", function(data) {
		if (data) {
			easyuiMask();
			ajaxRequest("POST", "",
					"platform/bpm/bpmPublishAction/reloadProcessDefinition", "json",
...
function setProcessDeploy(){
	...
	var deploymentId = data.deployId;
	var processDefId = data.processDefId;
	var url = getPath2()+"/avicit/platform6/bpmconsole/publish/ProcessDeployPage.jsp?deploymentId="+deploymentId+"&processDefId="+processDefId;
	top.addTab("流程配置管理",url,"static/images/platform/index/images/icons.gif","ProcessStartRight"," -0px -120px");
}
...
function selectUserDept() {
	...
	var usd = new UserSelectDialog('userSelectCommonDialog', 700, 400,
			getPath2() + '/platform/user/bpmSelectUserAction/userSelectCommon',
			'选择人员部门');
	var buttons = [ {
		text : '确定',
		id : 'processSubimt',
	...
	*/
	...
	/**
	var usd = new CommonDialog("userAddDialog","800","500",getPath2()+"/avicit/platform6/modules/system/sysgroup/addUser.jsp","选择用户");
	var buttons = [{
		text:'确定',
		id : 'formSubimt',
		//iconCls : 'icon-submit',
...
<div data-options="region:'west',title:'流程发布管理',split:true,collapsible:false"  style="width:150px;overflow: auto;">
	<ul id="mytree"> </ul>  
</div>  
...
<div id="toolbar" class="datagrid-toolbar" style="height:auto;display: block;">
			<fieldset>
				...
				<table class="tableForm" id="searchForm" width='100%'>
					<tr>
						<td>流程模板名称：</td>
						<td colspan="2"><input name="processName" id="processName"  editable="false" style="width: 150px;" class="easyui-validatebox"/>
						</td>
						<td>流程定义ID：</td>
						<td colspan="2"><input name="processDefId" id="processDefId"  editable="false" style="width: 150px;" class="easyui-validatebox"/>
						</td>
						<td>部署人：</td>
