...
function loadProcessEntry(){
	...
	$('#processentrylist').datagrid({
		url: 'platform/bpm/processAnalysisAction/getProcessInstanceListByPage.json?pdId=<%=pdId%>&userId=<%=userId%>&positionId=<%=positionId%>&startDateBegin=<%=startDateBegin%>&startDateEnd=<%=startDateEnd%>',
		width: '100%',
	    nowrap: false,
	    striped: true,
	    autoRowHeight:false,
