...
<script src="static/js/platform/component/fusionchar/js/FusionCharts.js" type="text/javascript"></script>
	...
	<script type="text/javascript">
		...
		function getTodoCompleteTop10Back(json) {
			...
			for (var i = 0; i < json.rows.length; i++) {
				strXML += "	<set label='"+json.rows[i].name+"' value='"+json.rows[i].value+"' link='JavaScript:openProcessTodoPage(\"TodoCompleteTop10\", \""+json.rows[i].id+"\")'/>" ;
			}
			strXML += "</chart>";
		...
		function getTodoStockTop10Back(json) {
			...
			for (var i = 0; i < json.rows.length; i++) {
				strXML += "	<set label='"+json.rows[i].name+"' value='"+json.rows[i].value+"' link='JavaScript:openProcessTodoPage(\"TodoStockTop10\", \""+json.rows[i].id+"\")'/>" ;
			}
				strXML += "</chart>";
			chart.setDataXML(strXML);
		...
		function getTodoCompleteAverageTimeTop10Back(json) {
			...
			for (var i = 0; i < json.rows.length; i++) {
				strXML += "	<set label='"+json.rows[i].name+"' value='"+json.rows[i].value+"' link='JavaScript:openProcessTodoPage(\"TodoCompleteAverageTimeTop10\", \""+json.rows[i].id+"\")'/>" ;
			}
				strXML += "</chart>";
			chart.setDataXML(strXML);
		...
		function getStartNumTop10Back(json) {
			...
			for (var i = 0; i < json.rows.length; i++) {
				strXML += "	<set label='"+json.rows[i].name+"' value='"+json.rows[i].value+"' link='JavaScript:openProcessUserStartPage(\"StartNumTop10\", \""+json.rows[i].id+"\")'/>" ;
			}
				strXML += "</chart>";
			chart.setDataXML(strXML);
		...
		function doQuery(tab){
		    ...
		    ajaxRequest("POST",data,"platform/bpm/bpmconsole/processUserAnalysisAction/get"+tab,"json","get"+tab+"Back");
		}
		...
		function doQueryAll(tab){
			var startDate =$("#"+tab+"Search input[comboname=startDate]").datetimebox('getValue');
			var endDate =$("#"+tab+"Search input[comboname=endDate]").datetimebox('getValue');
		    var url  = getPath2()+"/avicit/platform6/bpmconsole/analysis/ProcessUserAnalysisAll.jsp";
			var title = "统计信息";
			...
			top.addTab(title,url,"static/images/platform/index/images/icons.gif","ProcessUserAnalysisAll"," -0px -120px");
		}
		...
		function openProcessTodoPage(tab, id) {
			var url = getPath2()+"/avicit/platform6/bpmconsole/analysis/ProcessTodoList.jsp";
			var startDate =$("#"+tab+"Search input[comboname=startDate]").datetimebox('getValue');
			var endDate =$("#"+tab+"Search input[comboname=endDate]").datetimebox('getValue');
			...
			url += "?state="+state+"&orderBy="+orderBy+"&userId="+id+"&startDate="+startDate+"&endDate="+endDate;
			top.addTab("待办信息",url,"static/images/platform/index/images/icons.gif","ProcessTodoList"," -0px -120px");
		}
		
		//流程启动实例信息
		function openProcessUserStartPage(tab, id) {
			var url = getPath2()+"/avicit/platform6/bpmconsole/analysis/ProcessInstanceStartList.jsp";
			var startDateBegin =$("#"+tab+"Search input[comboname=startDate]").datetimebox('getValue');
			var startDateEnd =$("#"+tab+"Search input[comboname=endDate]").datetimebox('getValue');
			url += "?userId="+id+"&startDateBegin="+startDateBegin+"&startDateEnd="+startDateEnd;
			top.addTab("流程启动实例",url,"static/images/platform/index/images/icons.gif","ProcessInstanceStartList"," -0px -120px");
		}
		
		var processWaringDialog;
		//添加流程预警
		function doAddProcessWaring(id) {
			var para = "?waringType="+id+"&begin=当待办积压数量大于&end=条时";
			
			var usd = new UserSelectDialog("processWaringDialog","450","250",getPath2()+"/avicit/platform6/bpmconsole/analysis/ProcessWaringAdd.jsp"+encodeURI(para),"流程预警");
	...
	<body>
		<div class="easyui-tabs" id="processUserAnalysis">
			...
			<div title="流程待办积压数量最多TOP10" style="padding:10px;width:auto">
				...
				<div>
					<img src="static/images/platform/common/icons/table.png" title="列表统计" onclick="doQueryAll('TodoStockTop10')" style="cursor:pointer"/>
					<img src="static/images/platform/common/performance_warn.gif" title="流程预警" onclick="doAddProcessWaring('TodoStockTop10')" style="cursor:pointer"/>
				</div>
			</div>
