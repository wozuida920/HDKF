...
var pdid = '<%=pdid%>';
				...
				handler : function() {
					...
					if (tempFlag) {
						if (states == 'ended' || states == 'suspended') {
							...
							if (tempFlag) {
								dosupersede(processInstanceId, executionId, tempFlag, '', selectedTaskName);
							}
						}
					}
				}
				...
				handler : function() {
					...
					if (tempFlag) {
						if (states == 'ended' || states == 'suspended') {
							...
							if (tempFlag) {
								dotransmit(processInstanceId, executionId, tempFlag, '', '');
							}
						}
					}
				}
	...
	function backChecked(obj) {
		...
		if (obj != null && obj.success == true) {
			...
			var usd = new UserSelectDialog("qdlcghbb", "450", "200", encodeURI(url), "流程更换版本选择");
			usd.show();
		} else {
