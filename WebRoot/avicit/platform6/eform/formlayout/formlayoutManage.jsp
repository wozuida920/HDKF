...
<script src="avicit/platform6/eform/formdefine/js/eformCustomDialog.js"></script>
	...
	<script type="text/javascript">
		...
		function newObj() {
			...
			var  dlg = new CommonDialog("insert","680","400",url,"添加",false,true,false);
			dlg.show();
		}
		...
		function editObj() {
			...
			if(l>1){
				...
				if (row) {
					...
					var  dlg = new CommonDialog("update","680","400",url,"编辑",false,true,false);
					dlg.show();
				}else{
					$.messager.show({
						 title : '提示',
...
<div data-options="region:'center',split:true,border:false" style="padding:0px;height:0;overflow:hidden;">
	<!-- LIST表单 start-->
	<table id="dg"  class="easyui-datagrid"  url="platform/eform/cbbCRUDClient/list?tableId=${table.id}" fit="true" toolbar="#toolbar" pagination="true" rownumbers="true" fitColumns="true" singleSelect="true" checkOnSelect="true" selectOnCheck="false" autoRowHeight="false" striped="true"   data-options="onLoadSuccess:cellTip">
		<thead>
			<tr>
	...
	<div id="toolbar">
		...
		<sec:accesscontrollist hasPermission="3" domainObject="eform_toolbar_${table.tableName}_edit" permissionDes="编辑">
				<c:if test="${table.tableIsBpm== 'Y'}">
				    		<td><a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editObj()"   id='edit_button'   style="display:none">编辑</a></td>
				</c:if>
		...
		<sec:accesscontrollist hasPermission="3" domainObject="eform_toolbar_${table.tableName}_del" permissionDes="删除">
				    <c:if test="${table.tableIsBpm== 'Y'}">
				    		<td><a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="deleteObj()"  id='del_button'   style="display:none">删除</a></td>
				  </c:if>
					<c:if test="${table.tableIsBpm== 'N'}">
