...
$(document).ready(function(){ 
	...
	$('#dg1').datagrid({  
	    rowStyler:function(index,row){  
	        if (row.colName=='ID'||row.colName=='VERSION'||row.colName=='CREATION_DATE'||row.colName=='CREATED_BY'||row.colName=='LAST_UPDATED_BY'||row.colName=='LAST_UPDATE_DATE'||row.colName=='LAST_UPDATE_IP'){  
	            return 'color:blue';  
	        }  
	    }  
	});  
...
function editCol() {
	...
	if(l>1){
		...
		if (row) {
			url = 'platform/eform/tabledefine/initeditcol?id='+row.id;
	...
	if(row.tableIsCreated=='Y'){
		...
		if (row) {
			...
			if(colrow){
				...
				var tableId=row.id;
				url = 'platform/eform/tabledefine/initversion?tableId='+tableId+"&ids="+JSON.stringify(ids);
