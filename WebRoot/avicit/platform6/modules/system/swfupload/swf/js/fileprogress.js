...
function FileProgress(file,fileListID,swfUploadInstance,selectHtml,userName) {
	...
	**/
		...
		delObject.onclick = function () {
			...
			$("#"+fileListID+"fs").parent().parent().find(".lq-fieldest-title").html('附件(已添加'+filescount+'个附件)');},1000);
		};
		filescount++;
		$("#"+fileListID+"fs").parent().parent().find(".lq-fieldest-title").html('附件(已添加'+filescount+'个附件)'); 
		//alert(tb.innerHTML);
	}
	i++;
}
