...
<%
	...
	String lang = SessionHelper.getCurrentUserLanguageCode(request);
        ...
        String hiddenUploadBtn = "";
        hiddenUploadBtn = ComUtil.replaceNull2Space(request.getParameter("hiddenUploadBtn"));
...
var fileCategory = eval('('+'<%=fileCategory%>'+')');
var fileSecret = eval('('+'<%=fileSecret%>'+')');
...
<script type="text/javascript">
	...
	function upload(id){
        ...
        function formUpload(fun,secritValue){
            easyuiMask();//遮盖
            if (swfu.getStats().files_queued > 0) {
		...
		if (confirm("您要删除选中的附件吗?")) {
			...
			$.ajax({
				...
				success : function() {
					...
					filescount--;
					$("#"+form_code+"fs").parent().parent().find(".lq-fieldest-title").html('附件(已添加'+filescount+'个附件)');
				}
			});
		}
	}
				...
				<div id="uploadDiv">
					<table cellpadding="1" cellspacing="1">
						<tr style="font-size: 12px; font-weight: normal; color: aaaaaa;">
							...
							<td rowspan="2">
								<input id="btnUpload" type="button" onclick="upload();" value="上传" class="btn-upload">
							</td>
							<td>
								<%
...
<div id="uploadCountDiv" style="padding-top: 5px; display: none;">
	<div style="display: none">
		</span>等待上传 <span id="<%=form_code%>Count">0</span> 个 ，已上传 <span id="<%=form_code%>SuccessUploadCount">0</span> 个。&nbsp;&nbsp;
	</div>
