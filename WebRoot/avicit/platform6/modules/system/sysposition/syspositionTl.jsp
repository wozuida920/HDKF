...
function saveDeptTl(){
	...
	if(rows.length > 0){
		 $.ajax({
			 url:'platform/sysposition/sysPositionController/saveLanguageSet',
			 data : {datas : data, positionId:'${positionId}'},
			 type : 'post',
			 dataType : 'json',
	...
	if (rows.length > 0) {
		...
		$.messager.confirm('请确认','您确定要删除当前所选的数据？',function(b){
			if(b){
			   ...
			   $.ajax({
				 url:'platform/sysposition/sysPositionController/deleteSysPositionTl.json',
				 data : {
						ids : ids.join(',')
					},
				 type : 'post',
	...
	<div id="toolbarLanguage" >
					...
					<table id="dgDeptTl" class="easyui-datagrid"
							data-options="
								...
								checkOnSelect:true,
								selectOnCheck:true,
								url: 'platform/sysposition/sysPositionController/getSysPositionTlByParentId.json?id=${positionId}',
								method: 'post',
								fit : true,
								striped : false,
								idField :'id',
							...
							<thead>
								<tr>
									<th data-options="field:'sysPositionId', align:'center',align:'center',hidden:true" editor="{type:'text'}"></th>
									...
									<th data-options="field:'positionName',required:true,align:'center'" editor="{type:'text'}" width="220">岗位名称</th>
									<th data-options="field:'positionDesc',align:'center',align:'center'" editor="{type:'text'}"  width="220">岗位描述</th>
								</tr>
							</thead>
					</table>
</div>
