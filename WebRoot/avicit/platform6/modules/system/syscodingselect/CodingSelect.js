(function($){
	...
	function loadCodingData(target){
		...
		if(opts.codingId == null || opts.codingId == ''){
			if(opts.codingCode != null && opts.codingCode != ''){
				var url = getPath() +'/platform/sysCodingSelectController/getSysCodingBaseId.json';
				$.ajax({
			        cache: false,
			        type: "POST",
			        url: url,
	...
	function setDefaultValue(target){
		...
		if(opts.showDefaultValue == true && value == '' && opts.codingId != null && opts.codingId != ''){
			var url = getPath() +'/platform/sysCodingSelectController/getDefaultCodingValue.json';
			$.ajax({
		        cache: false,
		        type: "POST",
		        url: url,
	...
	$.fn.coding.parseOptions = function(target){
		...
		return $.extend({}, $.parser.parseOptions(target, [
			 'codingId','codingCode',{panelWidth:'number',panelHeight:'number', showDefaultValue:'boolean'}
		]), {
