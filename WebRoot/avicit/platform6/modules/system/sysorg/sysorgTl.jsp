...
function saveDeptTl(){
	...
	if(rows.length > 0){
		 $.ajax({
			 url:'platform/sysorg/sysOrgController/saveLanguageSet',
			 data : {datas : data, orgId:'${orgId}'},
			 type : 'post',
			 dataType : 'json',
	...
	if (rows.length > 0) {
		...
		$.messager.confirm('请确认','您确定要删除当前所选的数据？',function(b){
			if(b){
			   ...
			   $.ajax({
				 url:'platform/sysorg/sysOrgController/deleteSysOrgTl.json',
				 data : {
						ids : ids.join('@')
					},
				 type : 'post',
	...
	<div id="toolbarLanguage" >
					...
					<table id="dgDeptTl" class="easyui-datagrid"
							data-options="
								...
								checkOnSelect:true,
								selectOnCheck:true,
								url: 'platform/sysorg/sysOrgController/getListBySysOrgId.json?id=${orgId}',
								method: 'post',
								fit : true,
								striped : false,
								idField :'id',
							...
							<thead>
								<tr>
									<th data-options="field:'sysOrgId', align:'center',align:'center',hidden:true" editor="{type:'text'}"></th>
									...
									<th data-options="field:'orgName',required:true,align:'center'" editor="{type:'text'}" width="220">组织名称</th>
									<th data-options="field:'orgPlace',align:'center',align:'center'" editor="{type:'text'}"  width="220">办公地点</th>
									<th data-options="field:'orgDesc',align:'center',align:'center'" editor="{type:'text'}"  width="220">描述</th>
								</tr>
							</thead>
					</table>
</div>
