...
<div region="center" border="false">
   <form id="formSegmentValue" fit="true" style="padding: 5px 15px 40px 0;">
		...
		<table class="form_commonTable">
						...
						<td>
							<input id="segmentValue${numStatus.index}" segmentIndex="${num}" class="easyui-combobox" required="true"
								data-options="
									...
									editable:false,
									panelHeight:100,
									onShowPanel:comboboxOnShowPanel,
			...
			<tr>
				<th width="100px">码名称</th>
				<td><input id="segmentName" name="segmentName" class="easyui-validatebox" required="true" data-options="validType:'length[1,100]'" value="${segmentValues.segmentName }"/></td>
			</tr>
			<tr>
				<th>码值</th>
				<td>
					<c:choose>
						<c:when test="${segment.segmentLength gt 0}">
							<input id="segmentValue" name="segmentValue" class="easyui-validatebox" required="true" data-options="invalidMessage:'码值的长度必须为${segment.segmentLength }位',validType:'length[${segment.segmentLength },${segment.segmentLength }]'" value="${segmentValues.segmentValue }"/>
						</c:when>
						<c:otherwise>
