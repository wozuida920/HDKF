				...
				<form id="formRuleSegment_${segment.segmentIndex}" method="post" style="padding: 15px 25px 0 0;">
					...
					<table id="tableRuleSegment_${segment.segmentIndex}" class="form_commonTable">
						<tr>
							...
							<td colspan="3"><input id="segmentType_${segment.segmentIndex}" name="segmentType" class="easyui-combobox" required="true" data-options="editable:false,panelHeight:'auto',onShowPanel:comboboxOnShowPanel,onChange: function(newValue,oldValue){doChangeSegmentType(${segment.segmentIndex},newValue,oldValue)},valueField:'code',textField:'name',url:'platform/sysCodingRuleController/getSegmentType.json'" value="${segment.segmentType}"/></td>
						</tr>
						<tr>
							<th>码段长度</th>
							...
							<th width="6%">后缀</th>
							<td><input id="segmentSuffix_${segment.segmentIndex}" name="segmentSuffix" class="easyui-validatebox" data-options="required:false,validType:'length[0,100]'" value="${segment.segmentSuffix}"/></td>
						</tr>
						...
						<tr id="tdSegmentRelation_${segment.segmentIndex}" <c:if test="${segment.segmentType == '4' || segment.segmentType == '5' || (segment.segmentType == '1' && segment.comSegmentId != null) || (segment.segmentType == '2' && segment.comSegmentId != null)}">style="display: none"</c:if>>
							<th>分类码值关联<input type="hidden" id="segmentRelation_${segment.segmentIndex}" name="segmentRelation" value="${segment.segmentRelation}"/></th>
							<td colspan="7">
						...
						<tr id="tdCount_${segment.segmentIndex}" <c:if test="${segment.segmentType != '4'}">style="display: none"</c:if>>
							...
							<td colspan="3">
								<input id="format_${segment.segmentIndex}" name="format" class="easyui-combobox" data-options="editable:false,panelHeight:100,onShowPanel:comboboxOnShowPanel,valueField:'code',textField:'name',url:'platform/sysCodingRuleController/getCountFormatDate.json'" value="${segment.format}"/>
							</td>
