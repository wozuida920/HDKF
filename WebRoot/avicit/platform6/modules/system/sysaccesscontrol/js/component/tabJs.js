...
//	var dataGridHeight = $(".easyui-layout").height()-73;
	$('#roleList').datagrid({
		...
		fitColumns: true,
		queryParams: {currentComId: curComId},
 ...
 */
function insertUser(){
	if (curComId != "none" && curComId!="" && curComId!=null){
		var path="platform/sysAccessControlController/toAddUserJsp?type=component";
		var usd = new CommonDialog("insertUserDialog","700","435",path,"添加用户",false,true,false);
		usd.show();
	}else{
		$.messager.alert('提示', '请先选择需要授权的组件！', 'warning');
	}
}
...
function insertRole(){
	if (curComId != "none" && curComId!="" && curComId!=null){
		var path="platform/sysAccessControlController/toAddRoleJsp?type=component";
...
function insertGroup(){
	if (curComId != "none" && curComId!="" && curComId!=null){
		var path="platform/sysAccessControlController/toAddGroupJsp?type=component";
...
function insertPosition(){
	if (curComId != "none" && curComId!="" && curComId!=null){
		var path="platform/sysAccessControlController/toAddPositionJsp?type=component";
...
function selectDeptDialogCallBack(data){
	...
	$.ajax({
		 url:'platform/sysAccessControlController/insertComponentResourceAuth/D/'+curComId+'.json',
		 data:	JSON.stringify(ids),
		 contentType : 'application/json',
		 type : 'post',
		 dataType : 'json',
