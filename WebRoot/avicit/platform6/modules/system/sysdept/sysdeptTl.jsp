...
function saveDeptTl(){
	...
	if(rows.length > 0){
		 $.ajax({
			 url:'platform/sysdept/sysDeptController/saveLanguageSet',
			 data : {datas : data,deptId:'${deptId}'},
			 type : 'post',
			 dataType : 'json',
	...
	if (rows.length > 0) {
		...
		$.messager.confirm('请确认','您确定要删除当前所选的数据？',function(b){
			if(b){
			   ...
			   $.ajax({
				 url:'platform/sysdept/sysDeptController/batchDelLanguageSet.json',
				 data : {
						ids : ids.join('@')
					},
				 type : 'post',
	...
	<div id="toolbarLanguage" >
					...
					<table id="dgDeptTl" class="easyui-datagrid"
							data-options="
								...
								checkOnSelect:true,
								selectOnCheck:true,
								url: 'platform/sysdept/sysDeptController/getLanguageList.json?id=${deptId}',
								method: 'post',
								fit : true,
								striped : false,
								idField :'id',
							...
							<thead>
								<tr>
									<th data-options="field:'sysDeptId', align:'center',align:'center',hidden:true" editor="{type:'text'}"></th>
									...
									<th data-options="field:'deptName',required:true,align:'center'" editor="{type:'text'}" width="220">部门名称</th>
									<th data-options="field:'deptPlace',align:'center',align:'center'" editor="{type:'text'}"  width="220">办公地点</th>
									<th data-options="field:'deptDesc',align:'center',align:'center'" editor="{type:'text'}"  width="220">描述</th>
								</tr>
							</thead>
					</table>
</div>
