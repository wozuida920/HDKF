...
CsmpBdSparepart.prototype.init=function(){
	...
	this._datagrid=$(this._datagridId).datagrid({
		url:this.getUrl()+"getCsmpBdSparepartsByPage.json",
...
//确认回填
CsmpBdSparepart.prototype.setProductNameAnd = function() {
	var rows = this._datagrid.datagrid('getChecked');
	if (rows.length !== 1) {
		alert("请选择一条数据编辑！");
		return false;
	}
    parent.setSparePartTextAndChapter(rows[0]);
};
//详细页
CsmpBdSparepart.prototype.detail=function(id){
	this.nData = new CommonDialog("detail","790","500",this.getUrl()+'Detail/'+id,"详情",false,true,false);
	this.nData.show();
};
//保存功能
CsmpBdSparepart.prototype.save=function(form,id){
	var _self = this;
	$.ajax({
...
//删除
CsmpBdSparepart.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
...
//打开查询框
CsmpBdSparepart.prototype.openSearchForm =function(){
...
//隐藏查询框
CsmpBdSparepart.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/*清空查询条件*/
...
CsmpBdSparepart.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
    this._datagrid.datagrid('load',conditions);
};
                                                        CsmpBdSparepart.prototype.isRandompart=[];
Platform6.fn.lookup.getLookupType('CSMP_BD_RANDOMPART',function(r){r&&( CsmpBdSparepart.prototype.isRandompart=r);});  
