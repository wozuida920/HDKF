...
HdkfSupportOutfieldCard.prototype.init=function(){
	...
	this._datagrid=$(this._datagridId).datagrid({
		url:this.getUrl()+"getHdkfSupportOutfieldCardsDesignByPage.json",
...
//变更更改单标志页面
HdkfSupportOutfieldCard.prototype.change=function(){
	var rows = this._datagrid.datagrid('getChecked');
	if(rows.length !== 1){
		alert("请选择一条数据，修改更改单标志！");
		return false;
	}
	var url = "platform/hdkf/support/hdkfsupportdesignchange/HdkfSupportDesignChangeController/operation/";
	var params = rows[0].infoCardNo+","+rows[0].attribute01+","+rows[0].id;
	this.nData = new CommonDialog("changeDesign","790","500",url+'Add/'+params,"变更",false,true,false);
	this.nData.show();
};

HdkfSupportOutfieldCard.prototype.changeDetail=function(id){
	var url = "hdkf/support/hdkfsupportdesignchange/HdkfSupportDesignChangeController/HdkfSupportDesignChangeInfo?outCardId="+id;
	
	this.nData = new CommonDialog("changeDetail","790","500",url,"详情",false,true,false);
	this.nData.show();
...
//获取更改单号
HdkfSupportOutfieldCard.prototype.getdesignno=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
	for(var i=0;i<l;i++){
		if(!rows[i].attribute01||rows[i].attribute01=='N'){
			alert("包含未触发设计更改单记录，请重新选择！");
			return;
		}
		//alert("rows[i].attribute04-->"+rows[i].attribute04+"<---");
		if(rows[i].attribute04){
			alert("包含已经有设计更改单编号的记录，请重新选择！");
			return;
		}
	}
	...
	if(l > 0){
	 // $.messager.confirm('请确认','您确定要删除当前所选的数据？',function(b){
		// if(b){
			 ...
			 $.ajax({
				 url:_self.getUrl()+'updateDesignNo.json',
				 data:	JSON.stringify(ids),
				 contentType : 'application/json',
				 type : 'post',
				 dataType : 'json',
				 success : function(r){
					 if (r.flag == "success"&&r.noFlag!="success") {
						 _self.reLoad();
						 _self._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
						 $.messager.show({
							 title : '提示',
					...
					}else if(r.flag == "success"&&r.noFlag=="success") {
						 _self.reLoad();
						 _self._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
						 $.messager.show({
							 title : '提示',
							 msg : '包含未能获取设计更改单编号的记录！',
							 timeout: 15000  
						});
					}		 
...
HdkfSupportOutfieldCard.prototype.isDesign=[];
Platform6.fn.lookup.getLookupType('PLATFORM_YES_NO_FLAG',function(r){r&&( HdkfSupportOutfieldCard.prototype.isDesign=r);}); 
