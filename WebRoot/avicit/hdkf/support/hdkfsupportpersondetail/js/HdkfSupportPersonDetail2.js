...
HdkfSupportPersonDetail.prototype.init = function() {
	...
	this._datagrid = $(this._datagridId).datagrid({
		url : timestamp(this._url + 'getHdkfSupportPersonDetailsByPage.json'),
...
//打开借人单选择页面
function openDialogBorrowPersonInf() {
	var clientWidth = document.body.clientWidth;
	var clientHeight = document.body.clientHeight;
	this.nData = new CommonDialog("model4", clientWidth-200, clientHeight, "hdkf/support/hdkfsupportpersondetail/HdkfSupportPersonDetailController/HdkfSupportBorrowPersonInfSelect", "产品选择", false, true, false);
	this.nData.show();
};
function setBorrowPersonInfData(data) {
	$("#model4").dialog("close");
    $(selectborrowInfoId).val(data.id);
};
...
HdkfSupportPersonDetail.prototype.insert = function() {
	...
	$(ed.target).combobox('loadData', this.sex);
	 ...
	 var borrowInfoIdCell = temp.datagrid('getEditor',{index:0,field: 'borrowInfoId' });
	 selectborrowInfoId = borrowInfoIdCell.target;
	 $(borrowInfoIdCell.target).focus(function(){
		 openDialogBorrowPersonInf();
     });
};
// 保存功能
				...
				if (r.flag == "success") {
					//_self._datagrid.datagrid('reload', {});// 刷新当前页
					 _self._datagrid.datagrid({  
						    url:timestamp(_self._url+'getHdkfSupportPersonDetailsByPage.json'),
...
/*	this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid(
	...
	this._datagrid.datagrid('load',{ param : encodeURI(JSON.stringify(serializeObject(this.searchForm)))});
};
// 隐藏查询框
