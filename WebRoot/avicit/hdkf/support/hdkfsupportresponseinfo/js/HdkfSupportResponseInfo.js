 ...
 * 
 */
 function HdkfSupportResponseInfo(datagrid,url,searchD,form){
    ...
    this._formCode = "hdkfSupportResponseInfo"; //表单的code，启动流程时需要
...
//初始化操作
HdkfSupportResponseInfo.prototype.init=function(){
	var _self = this;
	...
	this._datagrid=$(this._datagridId).datagrid({
		url:this.getUrl()+"getHdkfSupportResponseInfosByPage.json",
...
//添加页面
HdkfSupportResponseInfo.prototype.insert=function(){
	this.nData = new CommonDialog("insert","790","500",this.getUrl()+'Add/null',"添加",false,true,false);
	this.nData.show();
};
//修改页面
HdkfSupportResponseInfo.prototype.modify=function(){
...
//详细页
HdkfSupportResponseInfo.prototype.detail=function(id, value, state ,process){
//	this.nData = new CommonDialog("edit","790","500",this.getUrl()+'Detail/'+id,"详情",false,true,false);
//	this.nData.show();
//	var url = this.getUrl()+'Detail/'+id;
	var strJsp = "HdkfSupportResponseInfoDetail.jsp";
	if (state != "结束")
	{
		if(process == "信息员答复;"){
			strJsp = "HdkfSupportResponseInfoDetailKf.jsp";
		}else{
			strJsp = "HdkfSupportResponseInfoDetailInfo.jsp";
		}
	}
	var url = "avicit/hdkf/support/hdkfsupportresponseinfo/" + strJsp + "?id=" + id;
...
//保存并启动流程
HdkfSupportResponseInfo.prototype.saveFormAndStartFlow=function(form,id){
	var _self = this;
...
//保存功能
HdkfSupportResponseInfo.prototype.save=function(form,id){
	var _self = this;
	$.ajax({
...
//删除
HdkfSupportResponseInfo.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
...
//重载数据
HdkfSupportResponseInfo.prototype.reLoad=function(){
	//this._datagrid.datagrid('load',{});
		this.searchData();
};
//关闭对话框
HdkfSupportResponseInfo.prototype.closeDialog=function(id){
	$(id).dialog('close');
};

//打开查询框
HdkfSupportResponseInfo.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
//去后台查询
HdkfSupportResponseInfo.prototype.searchData =function(){
...
//隐藏查询框
HdkfSupportResponseInfo.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/*清空查询条件*/
HdkfSupportResponseInfo.prototype.clearData =function(){
...
HdkfSupportResponseInfo.prototype.formate=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd");
	}
	return '';
};
HdkfSupportResponseInfo.prototype.formateDateTime=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd hh:mm:ss");
	}
	return '';
};
HdkfSupportResponseInfo.prototype.initWorkFlow=function(status, owner){
	setBpmMenuState(status,owner);
	$('#bpmState').val(status);
	$('#bpmType').val(owner);
	this.searchData();
};
HdkfSupportResponseInfo.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
    this._datagrid.datagrid('load',conditions);
};
                                    HdkfSupportResponseInfo.prototype.respBy=[];
Platform6.fn.lookup.getLookupType('SUPPORT_RESPONSE_TYPE',function(r){r&&( HdkfSupportResponseInfo.prototype.respBy=r);});  
