 ...
 * 
 */
 function HdkfSupportTripTask(datagrid,url,searchD,form){
    ...
    this._formCode = "hdkfSupportTripTask"; //表单的code，启动流程时需要
...
//初始化操作
HdkfSupportTripTask.prototype.init=function(){
	var _self = this;
	...
	this._datagrid=$(this._datagridId).datagrid({
		url:this.getUrl()+"getHdkfSupportTripTasksByPage.json",
...
//添加页面
HdkfSupportTripTask.prototype.insert=function(){
...
//修改页面
HdkfSupportTripTask.prototype.modify=function(){
...
//详细页
HdkfSupportTripTask.prototype.detail=function(id, value){
//	this.nData = new CommonDialog("edit","790","500",this.getUrl()+'Detail/'+id,"详情",false,true,false);
//	this.nData.show();
//	var url = this.getUrl()+'Detail/'+id;
	var url = "avicit/hdkf/support/hdkfsupporttriptask/HdkfSupportTripTaskDetail.jsp?id=" + id;
...
//保存并启动流程
HdkfSupportTripTask.prototype.saveFormAndStartFlow=function(form,id){
	//alert("saveFormAndStartFlow Func start....");
	var _self = this;
...
//保存功能
HdkfSupportTripTask.prototype.save=function(form,id){
	var _self = this;
	$.ajax({
...
//删除
HdkfSupportTripTask.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
...
//重载数据
HdkfSupportTripTask.prototype.reLoad=function(){
	//this._datagrid.datagrid('load',{});
		this.searchData();
};
//关闭对话框
HdkfSupportTripTask.prototype.closeDialog=function(id){
	$(id).dialog('close');
};

//打开查询框
HdkfSupportTripTask.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
//去后台查询
HdkfSupportTripTask.prototype.searchData =function(){
...
//隐藏查询框
HdkfSupportTripTask.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/*清空查询条件*/
HdkfSupportTripTask.prototype.clearData =function(){
...
HdkfSupportTripTask.prototype.formate=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd");
	}
	return '';
};
HdkfSupportTripTask.prototype.formateDateTime=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd hh:mm:ss");
	}
	return '';
};
HdkfSupportTripTask.prototype.initWorkFlow=function(status, owner){
	setBpmMenuState(status,owner);
	$('#bpmState').val(status);
	$('#bpmType').val(owner);
	this.searchData();
};
HdkfSupportTripTask.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
    this._datagrid.datagrid('load',conditions);
};
                    HdkfSupportTripTask.prototype.isNeedEquipment=[];
Platform6.fn.lookup.getLookupType('PLATFORM_YES_NO_FLAG',function(r){r&&( HdkfSupportTripTask.prototype.isNeedEquipment=r);});  
           HdkfSupportTripTask.prototype.isBorrowSomeone=[];
Platform6.fn.lookup.getLookupType('PLATFORM_YES_NO_FLAG',function(r){r&&( HdkfSupportTripTask.prototype.isBorrowSomeone=r);});  
