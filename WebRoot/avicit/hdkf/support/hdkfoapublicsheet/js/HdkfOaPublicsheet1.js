 ...
 * 
 */
 function HdkfOaPublicsheet(datagrid,url,searchD,form){
    ...
    this._formCode = "hdkfOaPublicsheet"; //表单的code，启动流程时需要
...
//初始化操作
HdkfOaPublicsheet.prototype.init=function(){
	var _self = this;
	...
	this._datagrid=$(this._datagridId).datagrid({
		url:this.getUrl()+"getHdkfOaPublicsheetsByPage1.json",
...
//添加页面
HdkfOaPublicsheet.prototype.insert=function(){
...
//修改页面
HdkfOaPublicsheet.prototype.modify=function(){
	...
	var clientWidth = document.body.clientWidth;
	var clientHeight = document.body.clientHeight;
	this.nData = new CommonDialog("edit",clientWidth,clientHeight,this.getUrl()+'Edit/'+rows[0].id,"编辑",false,true,false);
	this.nData.show();
};
//详细页
HdkfOaPublicsheet.prototype.detail=function(id, value){
//	this.nData = new CommonDialog("edit","790","500",this.getUrl()+'Detail/'+id,"详情",false,true,false);
//	this.nData.show();
//	var url = this.getUrl()+'Detail/'+id;
	var url = "avicit/hdkf/support/hdkfoapublicsheet/HdkfOaPublicsheetDetail.jsp?id=" + id;
...
//保存并启动流程
HdkfOaPublicsheet.prototype.saveFormAndStartFlow=function(form,id){
	var _self = this;
...
//保存功能
HdkfOaPublicsheet.prototype.save=function(form,id){
	var _self = this;
	$.ajax({
...
//删除
HdkfOaPublicsheet.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
...
//重载数据
HdkfOaPublicsheet.prototype.reLoad=function(){
	//this._datagrid.datagrid('load',{});
		this.searchData();
};
//关闭对话框
HdkfOaPublicsheet.prototype.closeDialog=function(id){
	$(id).dialog('close');
};

//打开查询框
HdkfOaPublicsheet.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
//去后台查询
HdkfOaPublicsheet.prototype.searchData =function(){
...
//隐藏查询框
HdkfOaPublicsheet.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/*清空查询条件*/
HdkfOaPublicsheet.prototype.clearData =function(){
...
HdkfOaPublicsheet.prototype.formate=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd");
	}
	return '';
};
HdkfOaPublicsheet.prototype.formateDateTime=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd hh:mm:ss");
	}
	return '';
};
HdkfOaPublicsheet.prototype.initWorkFlow=function(status, owner){
	setBpmMenuState(status,owner);
	$('#bpmState').val(status);
	$('#bpmType').val(owner);
	this.searchData();
};
HdkfOaPublicsheet.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
    this._datagrid.datagrid('load',conditions);
};


HdkfOaPublicsheet.prototype.attribute03=[];
Platform6.fn.lookup.getLookupType('PLATFORM_YES_NO_FLAG',function(r){r&&( HdkfOaPublicsheet.prototype.attribute03=r);}); 
