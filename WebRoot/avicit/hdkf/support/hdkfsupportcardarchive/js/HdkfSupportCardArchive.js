 ...
 * 
 */
 function HdkfSupportCardArchive(datagrid,url,searchD,form){
    ...
    this._formCode = "hdkfSupportCardArchive"; //表单的code，启动流程时需要
...
//初始化操作
HdkfSupportCardArchive.prototype.init=function(){
	var _self = this;
	...
	this._datagrid=$(this._datagridId).datagrid({
		url:this.getUrl()+"getHdkfSupportCardArchivesByPage.json",
...
//添加页面
HdkfSupportCardArchive.prototype.insert=function(){
...
//修改页面
HdkfSupportCardArchive.prototype.modify=function(){
...
//详细页
HdkfSupportCardArchive.prototype.detail=function(id, value){
//	this.nData = new CommonDialog("edit","790","500",this.getUrl()+'Detail/'+id,"详情",false,true,false);
//	this.nData.show();
//	var url = this.getUrl()+'Detail/'+id;
	var url = "avicit/hdkf/support/hdkfsupportcardarchive/HdkfSupportCardArchiveDetail.jsp?id=" + id;
...
//保存并启动流程
HdkfSupportCardArchive.prototype.saveFormAndStartFlow=function(form,id){
	var _self = this;
...
//保存功能
HdkfSupportCardArchive.prototype.save=function(form,id){
	var _self = this;
	$.ajax({
...
//删除
HdkfSupportCardArchive.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
...
//重载数据
HdkfSupportCardArchive.prototype.reLoad=function(){
	//this._datagrid.datagrid('load',{});
		this.searchData();
};
//关闭对话框
HdkfSupportCardArchive.prototype.closeDialog=function(id){
	$(id).dialog('close');
};

//打开查询框
HdkfSupportCardArchive.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
//去后台查询
HdkfSupportCardArchive.prototype.searchData =function(){
...
//隐藏查询框
HdkfSupportCardArchive.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/*清空查询条件*/
HdkfSupportCardArchive.prototype.clearData =function(){
...
HdkfSupportCardArchive.prototype.formate=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd");
	}
	return '';
};
HdkfSupportCardArchive.prototype.formateDateTime=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd hh:mm:ss");
	}
	return '';
};
HdkfSupportCardArchive.prototype.initWorkFlow=function(status, owner){
	setBpmMenuState(status,owner);
	$('#bpmState').val(status);
	$('#bpmType').val(owner);
	this.searchData();
};
HdkfSupportCardArchive.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
    this._datagrid.datagrid('load',conditions);
};
        HdkfSupportCardArchive.prototype.archiveType=[];
Platform6.fn.lookup.getLookupType('SUPPORT_ARCHIVE_TYPE',function(r){r&&( HdkfSupportCardArchive.prototype.archiveType=r);});  
