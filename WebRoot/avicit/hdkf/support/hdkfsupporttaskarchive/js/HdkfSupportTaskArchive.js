 ...
 * 
 */
 function HdkfSupportTaskArchive(datagrid,url,searchD,form){
    ...
    this._formCode = "hdkfSupportTaskArchive"; //表单的code，启动流程时需要
...
//初始化操作
HdkfSupportTaskArchive.prototype.init=function(){
	var _self = this;
	...
	this._datagrid=$(this._datagridId).datagrid({
		url:this.getUrl()+"getHdkfSupportTaskArchivesByPage.json",
...
//添加页面
HdkfSupportTaskArchive.prototype.insert=function(){
...
//修改页面
HdkfSupportTaskArchive.prototype.modify=function(){
...
//详细页
HdkfSupportTaskArchive.prototype.detail=function(id, value){
//	this.nData = new CommonDialog("edit","790","500",this.getUrl()+'Detail/'+id,"详情",false,true,false);
//	this.nData.show();
//	var url = this.getUrl()+'Detail/'+id;
	var url = "avicit/hdkf/support/hdkfsupporttaskarchive/HdkfSupportTaskArchiveDetail.jsp?id=" + id;
...
//保存并启动流程
HdkfSupportTaskArchive.prototype.saveFormAndStartFlow=function(form,id){
	var _self = this;
...
//保存功能
HdkfSupportTaskArchive.prototype.save=function(form,id){
	var _self = this;
	$.ajax({
...
//删除
HdkfSupportTaskArchive.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
...
//重载数据
HdkfSupportTaskArchive.prototype.reLoad=function(){
	//this._datagrid.datagrid('load',{});
		this.searchData();
};
//关闭对话框
HdkfSupportTaskArchive.prototype.closeDialog=function(id){
	$(id).dialog('close');
};

//打开查询框
HdkfSupportTaskArchive.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
//去后台查询
HdkfSupportTaskArchive.prototype.searchData =function(){
...
//隐藏查询框
HdkfSupportTaskArchive.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/*清空查询条件*/
HdkfSupportTaskArchive.prototype.clearData =function(){
...
HdkfSupportTaskArchive.prototype.formate=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd");
	}
	return '';
};
HdkfSupportTaskArchive.prototype.formateDateTime=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd hh:mm:ss");
	}
	return '';
};
HdkfSupportTaskArchive.prototype.initWorkFlow=function(status, owner){
	setBpmMenuState(status,owner);
	$('#bpmState').val(status);
	$('#bpmType').val(owner);
	this.searchData();
};
HdkfSupportTaskArchive.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
