 ...
 * 
 */
 function HdkfSupportBorrowPersonInf(datagrid,url,searchD,form){
    ...
    this._formCode = "hdkfSupportBorrowPersonInf"; //表单的code，启动流程时需要
...
//初始化操作
HdkfSupportBorrowPersonInf.prototype.init=function(){
	var _self = this;
	...
	this._datagrid=$(this._datagridId).datagrid({
		url:this.getUrl()+"getHdkfSupportBorrowPersonInfsByPage.json",
...
//添加页面
HdkfSupportBorrowPersonInf.prototype.insert=function(){
	var clientWidth = document.body.clientWidth;
	var clientHeight = document.body.clientHeight;
	this.nData = new CommonDialog("insert",clientWidth-200,clientHeight,this.getUrl()+'Add/null',"添加",false,true,false);
	this.nData.show();
};

//修改页面
HdkfSupportBorrowPersonInf.prototype.modify=function(){
	...
	var clientWidth = document.body.clientWidth;
	var clientHeight = document.body.clientHeight;
	this.nData = new CommonDialog("edit",clientWidth-200,clientHeight,this.getUrl()+'Edit/'+rows[0].id,"编辑",false,true,false);
	this.nData.show();
};
//详细页
HdkfSupportBorrowPersonInf.prototype.detail=function(id, value){
//	this.nData = new CommonDialog("edit","790","500",this.getUrl()+'Detail/'+id,"详情",false,true,false);
//	this.nData.show();
//	var url = this.getUrl()+'Detail/'+id;
	var url = "avicit/hdkf/support/hdkfsupportborrowpersoninf/HdkfSupportBorrowPersonInfDetail.jsp?id=" + id;
...
//保存并启动流程
HdkfSupportBorrowPersonInf.prototype.saveFormAndStartFlow=function(form,id){
...
//保存功能
HdkfSupportBorrowPersonInf.prototype.save=function(form,id){
	var phoneReg = /^1[3,4,5,8]{1}[0-9]{1}[0-9]{8}$/;
	var telReg1 = /^[0-9]{3}-[0-9]{8}$/;
	var telReg2 = /^[0-9]{4}-[0-9]{7}$/;
	var flag = 0;
...
//删除
HdkfSupportBorrowPersonInf.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
...
//重载数据
HdkfSupportBorrowPersonInf.prototype.reLoad=function(){
	//this._datagrid.datagrid('load',{});
		this.searchData();
};
//关闭对话框
HdkfSupportBorrowPersonInf.prototype.closeDialog=function(id){
	$(id).dialog('close');
};

//打开查询框
HdkfSupportBorrowPersonInf.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
};
//去后台查询
HdkfSupportBorrowPersonInf.prototype.searchData =function(){
...
//隐藏查询框
HdkfSupportBorrowPersonInf.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
};
/*清空查询条件*/
HdkfSupportBorrowPersonInf.prototype.clearData =function(){
...
HdkfSupportBorrowPersonInf.prototype.formate=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd");
	}
	return '';
};
HdkfSupportBorrowPersonInf.prototype.formateDateTime=function(value){
	if(value){
		return new Date(value).format("yyyy-MM-dd hh:mm:ss");
	}
	return '';
};
HdkfSupportBorrowPersonInf.prototype.initWorkFlow=function(status, owner){
	setBpmMenuState(status,owner);
	$('#bpmState').val(status);
	$('#bpmType').val(owner);
	this.searchData();
};
HdkfSupportBorrowPersonInf.prototype.searchDataBySfn =function(conditions){
    this._datagrid.datagrid('uncheckAll').datagrid('unselectAll').datagrid('clearSelections');
