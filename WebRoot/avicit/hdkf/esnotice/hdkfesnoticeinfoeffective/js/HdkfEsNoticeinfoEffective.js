...
HdkfEsNoticeinfoEffective.prototype.init = function() {
	...
	this._datagrid = $(this._datagridId).datagrid(
			{
				url : this._url + 'getHdkfEsNoticeinfoEffectivesByPage/'
						+ bulletinIdR + '/getPage',
...
HdkfEsNoticeinfoEffective.prototype.edit = function() {
	...
	if (!rows[0].doFlag) {
		...
		/**
		 * 通用代码 var ed = temp.datagrid('getEditor',{index:index,field:
		 * 'validFlag' }); $(ed.target).combobox( 'loadData' ,
						...
						onSelect : function(r) {
							...
							if (ynFlag == 'Y') {
								...
								$(ed3.target).datebox('setValue',
										this.completeDate).datebox('setValue',
										completeDate);
								$(ed4.target).val("");
								$(ed4.target).attr("disabled", true);
...
Platform6.fn.lookup.getLookupType('PLATFORM_YES_NO_FLAG', function(r) {
	r && (HdkfEsNoticeinfoEffective.prototype.doFlag = r);
}, true);
